...
namespace Teleperformance.DataIngestion.DataAccess.Repository.v1._0
{
    public class ProcessConfigurationRepository : IProcessConfigurationRepository
    {
        ...
        public async Task<string?> InsertFlpConfigurationDetails(InsertFlpConfigurationRequest insertFlpConfigurationRequest)
        {
            try
            {
                var dynamicParameters = FlpConfigurationInsertParameterBuilder.Build(insertFlpConfigurationRequest);
                await _dapperService.InsertDataAsync<string>("[commit_InsertFlpConfiguration]", dynamicParameters, commandType: CommandType.StoredProcedure);

                var newFlpConfigurationId = FlpConfigurationInsertParameterBuilder.ReadConfigurationId(dynamicParameters);
