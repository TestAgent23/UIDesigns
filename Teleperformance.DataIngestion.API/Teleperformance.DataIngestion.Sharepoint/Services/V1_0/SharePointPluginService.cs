...
public class SharePointPluginService : ISharePointPluginService
{
    ...
    public async Task<Application> SaveApplicationAsync(Application application)
    {
        ...
        if (application.RegionIdent is null or <= 0)
            throw new ArgumentException(ApplicationRegionRequired);
        ...
        if (string.Equals(application.ApplicationTypeCode, "tp_internal", StringComparison.OrdinalIgnoreCase) && isActive)
    ...
    public async Task<bool> MoveFileAsync(WorkspaceCredentials credentials, string sourceFilePath, string destinationFolderPath, string newFileName, bool createDestinationIfMissing = false)
    {
        ...
        // 2. Resolve destination folder — create missing segments only when FLP ingest requests it
        string destFolderItemId;
        if (createDestinationIfMissing)
        {
            destFolderItemId = await EnsureFolderPathExistsAsync(resolved, siteId, driveId, destinationFolderPath).ConfigureAwait(false);
        }
        else
        {
            ...
            using var destFolderDoc = await SendGraphGetAsync(destFolderUrl, resolved).ConfigureAwait(false);
            destFolderItemId = CommonUtilities.JsonGetString(destFolderDoc.RootElement, "id")
                ?? throw new InvalidOperationException($"Cannot find destination folder: {destinationFolderPath}");
        }
