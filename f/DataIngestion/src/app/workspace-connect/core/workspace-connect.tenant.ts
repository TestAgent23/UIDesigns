...
export function tenantHierarchySelectionFromDto(app: {
  ...
  const securityGroupId = app.securityGroupId && app.securityGroupId !== SP_ALL_SECURITY_GROUP_ID ? app.securityGroupId : null;
  const securityGroupName = app.securityGroupName ?? SP_ALL_SECURITY_GROUP_NAME;
  return {
    ...
    securityGroupId,
    securityGroupName,
    securityGroups: securityGroupId ? [{ id: securityGroupId, name: securityGroupName }] : [],
  };
}
...
export function normalizeTenantHierarchyForSave(
  ...
  const primarySecurityGroup = selection.securityGroups?.[0];
  const securityGroupId = selection.securityGroupId?.trim() || primarySecurityGroup?.id?.trim() || SP_ALL_SECURITY_GROUP_ID;
  const securityGroupName = selection.securityGroupName?.trim() || primarySecurityGroup?.name?.trim() || SP_ALL_SECURITY_GROUP_NAME;
  return {
