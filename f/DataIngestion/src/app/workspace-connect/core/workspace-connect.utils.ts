...
const TYPE_LABELS = WC_CONNECTORS.typeFallback;

export const FALLBACK_APPLICATION_TYPES: ApplicationTypeDto[] = [
  { applicationTypeId: 1, code: 'tp_external', displayName: TYPE_LABELS.external },
  { applicationTypeId: 2, code: 'tp_internal', displayName: TYPE_LABELS.internal },
  { applicationTypeId: 3, code: 'tp_user_delegated', displayName: TYPE_LABELS.userDelegated },
];
...
const REGISTRATION_TYPE_CODES = ['tp_internal', 'tp_external'] as const;
...
export function registrationTypeOptions(types: ApplicationTypeDto[]): ApplicationTypeDto[] {
  return REGISTRATION_TYPE_CODES.map(
    (code) => applicationTypeByCode(types, code) ?? applicationTypeByCode(FALLBACK_APPLICATION_TYPES, code)!,
  );
}
...
export function applicationFormFromDto(app: ApplicationDto): ApplicationFormModel {
  ...
  return {
    ...
    sites,
    tenantHierarchy: tenantHierarchySelectionFromDto(app),
    isActive: app.isActive !== false,
    isDraft: app.isDraft === true,
  };
}
