...
import { cleanColumnName, convertToRoman, DiParserService } from '../core/services/di-parser.service';
...
import { LandingLayerConfiguration, LandingLayerInsertConfigurationRequest, SelectedFiles } from '../core/models/LandingLayer/landingLayer';
...
import { wcImageUrl } from '../workspace-connect/core/workspace-connect.assets';
...
export class FileUploadComponent implements OnInit, AfterViewInit, AfterViewChecked {
  ...
  fileSourceSelected = false;
  selectedFileSource: 'local' | 'sharepoint' | null = null;
  sharepointToPreviewLoading = false;
  readonly spFileFirst = SP_INTEGRATION.fileFirst;
  readonly spWorkspaceNavTitle = 'SharePoint Workspace';
  readonly spSharepointIcon = wcImageUrl('sharepoint-mark.svg');
  // #endregion
  selectedTab = 0;
  ...
  columnNameDisplayMode: 'ColumnName' | 'DbColumnName' = 'ColumnName';
  dbColumnNameEditModeByIndex: { [key: number]: boolean } = {};
  dbColumnNameDraftByIndex: { [key: number]: string } = {};
  ...
  setColumnNameDisplayMode(mode: 'ColumnName' | 'DbColumnName'): void {
    this.columnNameDisplayMode = mode;
  }

  isDbColumnNameDisplayMode(): boolean {
    return this.columnNameDisplayMode === 'DbColumnName';
  }

  onToggleDbColumnNameDisplay(checked: boolean): void {
    this.columnNameDisplayMode = checked ? 'DbColumnName' : 'ColumnName';
  }

  isDbColumnNameEditMode(index: number): boolean {
    return !!this.dbColumnNameEditModeByIndex[index];
  }

  getDbColumnNameDraft(index: number, fallback: string): string {
    const draft = this.dbColumnNameDraftByIndex[index];
    return draft !== undefined ? draft : fallback;
  }

  onDbColumnNameDraftChange(index: number, value: string): void {
    this.dbColumnNameDraftByIndex[index] = value;
  }

  async onDbColumnNameEditAction(index: number, isEditing: boolean): Promise<void> {
    if (!isEditing) {
      const column = this.columnDatatype.find((x) => x.index === index);
      if (column) {
        this.dbColumnNameDraftByIndex[index] = column.DbColumnName;
      }
      this.dbColumnNameEditModeByIndex[index] = true;
      return;
    }

    const draft = this.dbColumnNameDraftByIndex[index];
    if (draft !== undefined) {
      await this.onDBColumnNameChange({ target: { value: draft } }, index);
    }

    this.dbColumnNameEditModeByIndex[index] = false;
  }

  onToggleDbColumnNameEditMode(index: number, checked: boolean): void {
    this.dbColumnNameEditModeByIndex[index] = checked;
  }

  getHeaderDisplayName(header: ColumnNameDatatypeName): string {
    if (this.columnNameDisplayMode === 'DbColumnName') {
      return header.DbColumnName || header.ColumnName;
    }

    return header.ColumnName || header.DbColumnName;
  }
  ...
  get showSourceChooser(): boolean {
    return !this.fileSourceSelected && !this.isFileUploaded;
  }

  get activeSourceLabel(): string {
    return this.selectedFileSource === 'sharepoint' ? this.spWorkspaceNavTitle : this.spFileFirst.localUpload;
  }

  startOverSourceSelection(): void {
    this.fileSourceSelected = false;
    this.selectedFileSource = null;
    this.showSharepoint = false;
    this.sharepointToPreviewLoading = false;
    this.isFileUploaded = false;
    ...
    this.wksheets = [];
    if (this.config) {
      this.config.fileType = FileType.CommaSeparatedValues;
    }
    this.fileUploadForm.get('configuration')?.setValue(null);
    if (this.formFile?.nativeElement) {
  ...
  onSharepointFileSelected(file: File): void {
    this.sharepointToPreviewLoading = true;
    this.showSharepoint = false;
    setTimeout(() => {
      void this.onFileChange({ target: { files: [file] }, fromSharepoint: true });
    }, 0);
  }
  // #endregion
  ...
  onTabChanged(tabName: string, event: any) {
    ...
    if (this.wksheets.length > 1) {
      ...
      [...this.wksheets].reverse().forEach(wk => {
        if (wk.ignoreSheet === false && wk.newSheet === false) {
  ...
  async onFileChange(event: any) {
    const fromSharepoint = !!event?.fromSharepoint;
    ...
    if (event.target.files.length > 0) {
      ...
      await this.onPreview(this.config);
      this.sharepointToPreviewLoading = false;
  ...
  resetForm(preserveClientInfo: boolean = false) {
  ...
  resetFormByChangeMapping(selectedExistingProcess: boolean = false, preserveClientInfo: boolean = this.selectedFileSource === 'sharepoint') {
  ...
  updateConfigOnly([additionalSettingsVal, columnData, ruleSet, ruleSetNames, formGroup]: [AdditionalSettings, ColumnNameDatatypeName[], ExcelRule[], RuleSetNames[], FormGroup]) {
    if (additionalSettingsVal) {
      ...
      if (this.selectedWorkSheetName) {
        ...
        this.columnDatatype = columnData;
    ...
    switch (this.fileType) {
      ...
      case FileType.MSExcel3:
        ...
        if (parsedExcel) {
          ...
          // Filter out sheets with empty workbooks
          this.wksheets = parsedExcel.filter(wk =>
            wk.sheetName &&
            wk.sheetName.trim().length > 0 &&
            wk.workBook &&
            wk.workBook.length > 0  // Add this condition
          );


          // Check if we have any valid sheets
          if (this.wksheets.length === 0) {
            this.toastr.error('No data found in any sheets. Please check your  file.');
            this.busyService.idle();
            this.resetForm();
            return null;
          }
  ...
  async onDBColumnNameChange(event: any, index: number) {
    ...
    if (column.DbColumnName === input.value) {
      return;
    }
    ...
    let listOfRulesColumnBelongs: ExcelRule[] = tempRuleSet?.filter((x) => {
      const matchedInRuleColumnName = x.ruleColumnName?.includes(column.ColumnName);
      const matchedInRuleColumnName2 = x.ruleColumnName2 === column.ColumnName;

      const escapedColumnName = this.helperUtil.escapeRegExp(column.ColumnName);
      const re = new RegExp(`(^|[^@])@${escapedColumnName}\\b`, 'i');
      const matchedInPrompt = re.test(x.prompt ?? '');

      return matchedInRuleColumnName || matchedInRuleColumnName2 || matchedInPrompt;
    }) ?? [];

    console.log(listOfRulesColumnBelongs);

    if (listOfRulesColumnBelongs.length > 0) {
      //look for the rule that has GenericRules
      // var itemsWithGenericRules = listOfRulesColumnBelongs.filter(x => x.ruleTypeId === RuleTypeNames.GenericRules);
      // if (itemsWithGenericRules.length > 0) {
      //   columnHasRule = itemsWithGenericRules[0];


      //   const confirmed = await this.confirmModalService2.confirm('Validations', `Column ${column.ColumnName} is part of a rule set.<br> If changed, the rule will be removed. Would you like to proceed?`);
      ...
      //   //remove all the genericrule in this.childComponent.excelFileRule using itemswithgeneric
      //   itemsWithGenericRules.forEach(item => {
      //     this.childComponent.excelFileRule = this.childComponent?.excelFileRule.filter(x => x.id !== item.id);
    ...
    listOfRulesColumnBelongs.forEach(rule => {
      switch (rule.ruleTypeId) {
        case RuleTypeNames.Custom: {
          // Replace @<old column name> with @<new DB column name> in both prompt fields.
          const escapedOldName = this.helperUtil.escapeRegExp(column.ColumnName);
          const reOld = new RegExp(`@${escapedOldName}\\b`, 'gi');
          // if (rule.prompt) {
          //   rule.prompt = rule.prompt.replace(reOld, `@${columnName}`);
          // }
          if (rule.promptUsingDBColumnName) {
            rule.promptUsingDBColumnName = rule.promptUsingDBColumnName.replace(reOld, `@${columnName}`);
          }
          break;
        }
        case RuleTypeNames.Value:
          if (rule.subRuleId === SubRuleTypes.Comparison) {
            //find if column.DbColumnName is in ruleDbColumnNames or is in ruleDbColumnNames2
            let isRuleDbColumnNames = rule.ruleDbColumnNames.find(x => x === column.DbColumnName);
            let isRuleDbColumnNames2 = rule.ruleDbColumnNames2 === column.DbColumnName;
            if (isRuleDbColumnNames) {
              //change the value using the columnName
              rule.ruleDbColumnNames[0] = columnName;
            }

            if (isRuleDbColumnNames2) {
              rule.ruleDbColumnNames2 = columnName;
            }
          }

          if ([SubRuleTypes.ExactMatch, SubRuleTypes.Numeric, SubRuleTypes.NumericRange].includes(rule.subRuleId)) {
            rule.ruleDbColumnNames[index] = columnName;
          }
          break;
        case RuleTypeNames.Format:
          rule.ruleDbColumnNames[index] = columnName;
          break;
    ...
    console.log(columnHasRule);
    column.DbColumnName = columnName;
    let indexOfColumnToChange = -1;
    ...
    if (this.config.spanish_to_english) {
      ...
      column.DbColumnName = this.helperUtil.convertToEnglishOnlyCharactersPerWord(
        this.englishOnlyCharacters,
        columnName
      );
      //change the column in the columnHasRule.ruleDbColumnNames
      //1) columnHasRule is the rule to change
      //2) find the column index to be changed
      listOfRulesColumnBelongs.forEach(rule => {
        rule.ruleDbColumnNames[index] = column.DbColumnName;
      });
    }
    ...
    // Keep edit draft aligned with the final normalized value shown in UI.
    this.dbColumnNameDraftByIndex[index] = column.DbColumnName;
  }
              ...
              if (response.result !== null) {
                ...
                for (const fileSetting of fileSettings) {
                  ...
                  var settingsFromExistingFileConfiguration: AdditionalSettings = {
                    ...
                    databaseNameDisplayName: '', //to be retrieved based on databaseConfigurationId
                    deltaStorageAccountIdDisplayName: '', //to be retrieved based on deltaStorageAccountId
                  };
  ...
  partitionBy(data1: any[], key: string[], orderKey: string) {
    ...
    const winners = Object.values(groups).map((group: any[]) => {
