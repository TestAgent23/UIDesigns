...
import { catchError, firstValueFrom, map, Observable, of, shareReplay, timeout } from 'rxjs';
...
import { FileNameExtension } from '../models/LandingLayer/landingLayer';
...
export class ConfigurationService {
  ...
  getConfigurationDetailsV2(configurationId: string, tabName: string) {
    ...
    // SharePoint tabName is a UI navigation key and should not be used as API tab filter.
    if (normalizedTabName && normalizedTabName !== 'sharepoint-settings-tab') {
      url += `&tabName=${normalizedTabName}`;
    }
    return this._http.get<APIResponse<FileConfigurationDetails>>(url, this.httpHeader());
  }

  // #region Sharepoint Workspace - AY
  getSharePointSourceByConfigId(configurationId: string) {
    return this._http.get<APIResponse<FlpSharePointSourceDetail>>(
      `${this.baseUrl}api/ProcessConfiguration/GetSharePointSourceByConfigId?configurationId=${configurationId}`,
      this.httpHeader()
    );
  }
  // #endregion
