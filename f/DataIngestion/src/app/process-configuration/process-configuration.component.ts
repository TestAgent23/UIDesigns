...
import { ChangeDetectionStrategy, Component, DoCheck, ElementRef, EventEmitter, Input, IterableDiffers, OnChanges, OnInit, Output, SimpleChanges, ViewChild } from '@angular/core';
import { AbstractControl, AsyncValidatorFn, FormArray, FormBuilder, FormControl, FormGroup, Validators } from '@angular/forms';
import { ToastrService } from 'ngx-toastr';
import { BehaviorSubject, debounceTime, distinctUntilChanged, EMPTY, filter, finalize, firstValueFrom, map, Observable, of, Subject, Subscription, switchMap, take, tap } from 'rxjs';
import { AdditionalSettings, RegexItem } from '../core/models/additionalSettings';
import { APIResponse } from '../core/models/apiResponse';
import { APIDrawflowNode, APINodeMapping, DrawFlowProperties } from '../core/models/APISource/drawFlow';
import { ColumnNameDatatypeName } from '../core/models/columnNameDatatypeName';
import { ConditionalOperator, DataAssistsRequest, ExcelRule, LogicalOperator, Patterns, RuleSetNames, RuleType, RuleTypes, SPNames, SubRule } from '../core/models/DataInsider';
...
import { DataSourceType, FileType, ModalMessages, RuleTypeNames, SubRuleTypes, ToastrMessages } from '../shared/enum';
...
export class ProcessConfigurationComponent implements OnInit, OnChanges, DoCheck {
  ...
  @Input() allNodes: APIDrawflowNode[] = [];
  @Input() nodeProperties: DrawFlowProperties[] = [];
  ...
  getNodeProperty(nodeName: string): DrawFlowProperties | undefined {
    return this.nodeProperties?.find(p => p.name === nodeName);
  }

  private normalizeNodeName(value: string | undefined | null): string {
    return (value ?? '').toLowerCase().replace(/[^a-z0-9]/g, '');
  }

  private isExcludedApiSourceNode(node: APIDrawflowNode | undefined): boolean {
    const normalizedName = this.normalizeNodeName(node?.name);
    return normalizedName === 'authentication' || normalizedName === 'endpointloop';
  }

  get filteredApiSourceNodes(): APIDrawflowNode[] {
    return (this.allNodes ?? []).filter(node => !this.isExcludedApiSourceNode(node));
  }

  expandedNodeIds = new Set<number>();
  selectedNodeIds = new Set<number>();
  selectedPathsByNode = new Map<number, Set<string>>();

  selectedNodeMappings: APINodeMapping[] = [];
  @Output() nodeMappingsChange = new EventEmitter<APINodeMapping[]>();

  /** Navigate a simple dot-separated path (no wildcards) */
  private getValueBySimplePath(obj: any, path: string): any {
    if (!obj || !path) return obj;
    return path.split('.').reduce((acc, key) => (acc != null ? acc[key] : undefined), obj);
  }

  private toColumnModel(columnName: string, index: number): ColumnNameDatatypeName {
    const existing = this.columnDatatype?.find(c => c.ColumnName === columnName || c.DbColumnName === columnName);
    if (existing) {
      return { ...existing };
    }
    ...
    return {
      ...
      willAddNewColumn: false,
      invalidColumnName: false,
      columnForDedeup: false,
      dateTimeFormatId: 0,
      isDuplicateColumn: false,
      useMultipleSheets: false
    };
  }

  private toCleanMappingColumnName(value: string, index: number): string {
    const leaf = (value ?? '').replace(/^\./, '').split('.').pop() ?? '';
    const cleaned = cleanColumnName(leaf);
    return cleaned && cleaned.trim().length > 0 ? cleaned : `COL${index}`;
  }

  private syncMappings(): void {
    const result: APINodeMapping[] = [];

    this.selectedPathsByNode.forEach((paths, nodeId) => {
      const node = this.allNodes.find(n => n.id === nodeId);
      if (this.isExcludedApiSourceNode(node)) {
        return;
      }
      const nodeProperty = node ? this.getNodeProperty(node.name) : undefined;
      const response = nodeProperty?.response;
      const nodeName = node?.name ?? `Node ${nodeId}`;

      // Group paths by their array container prefix (part before [*].)
      ...
      paths.forEach(path => {
        ...
        if (starIdx !== -1) {
          const arrayPath = path.substring(0, starIdx);
          const leafField = path.substring(starIdx + 4) || path;
          if (!arrayGroups.has(arrayPath)) arrayGroups.set(arrayPath, []);
          arrayGroups.get(arrayPath)!.push(leafField);
        } else {
          scalarPaths.push(path);
        }
      });

      // One table group per array container
      arrayGroups.forEach((fields, arrayPath) => {
        const arr = response ? this.getValueBySimplePath(response, arrayPath) : [];
        const rows: Record<string, any>[] = Array.isArray(arr)
          ? arr.map(item => {
            const row: Record<string, any> = {};
            fields.forEach((f, index) => {
              const normalizedField = (f ?? '').replace(/^\./, '');
              const cleanedName = this.toCleanMappingColumnName(normalizedField, index);
              row[cleanedName] = this.getValueBySimplePath(item, normalizedField);
            });
            return row;
          })
          : [];
        const columns = fields.map((field, index) => this.toColumnModel(this.toCleanMappingColumnName(field, index), index));
        result.push({ nodeId, nodeName, columns, rows });
      });

      // Scalar (non-array) paths as a single-row group
      if (scalarPaths.length > 0) {
        const row: Record<string, any> = {};
        const columnNames: string[] = [];
        scalarPaths.forEach((p, index) => {
          const label = this.toCleanMappingColumnName(p, index);
          columnNames.push(label);
          row[label] = response ? this.getValueBySimplePath(response, p) : undefined;
        });
        const columns = columnNames.map((name, index) => this.toColumnModel(name, index));
        result.push({ nodeId, nodeName, columns, rows: [row] });
      }
    });

    this.selectedNodeMappings = result;
    this.nodeMappingsChange.emit(this.selectedNodeMappings);
  }
  ...
  onNodeCheckChange(nodeId: number, checked: boolean, paths: string[]): void {
    if (checked) {
      this.selectedNodeIds.add(nodeId);
      this.selectedPathsByNode.set(nodeId, new Set(paths));
    } else {
      this.selectedNodeIds.delete(nodeId);
      this.selectedPathsByNode.delete(nodeId);
    }
    this.syncMappings();
  }

  onPathCheckChange(nodeId: number, path: string, checked: boolean): void {
    if (!this.selectedPathsByNode.has(nodeId)) {
      this.selectedPathsByNode.set(nodeId, new Set());
    }
    const paths = this.selectedPathsByNode.get(nodeId)!;
    if (checked) {
      paths.add(path);
      this.selectedNodeIds.add(nodeId);
    } else {
  ...
  isPathSelected(nodeId: number, path: string): boolean {
    return this.selectedPathsByNode.get(nodeId)?.has(path) ?? false;
  }

  buildResponseTree(obj: any, prefix = ''): ResponseTreeNode[] {
    if (!obj || typeof obj !== 'object') return [];
    if (Array.isArray(obj)) {
      return obj.length > 0 ? this.buildResponseTree(obj[0], prefix) : [];
    }
    ...
    for (const key of Object.keys(obj)) {
      ...
      if (val !== null && val !== undefined && typeof val === 'object') {
        if (Array.isArray(val)) {
          const arrKey = `${key}[*]`;
          const arrPath = `${path}[*]`;
          const children = val.length > 0 ? this.buildResponseTree(val[0], arrPath) : [];
          nodes.push({ key: arrKey, path: arrPath, children });
        } else {
          nodes.push({ key, path, children: this.buildResponseTree(val, path) });
        }
      } else {
  ...
  expandedTreePaths = new Map<number, Set<string>>();

  toggleTreeNode(nodeId: number, path: string): void {
    if (!this.expandedTreePaths.has(nodeId)) {
      this.expandedTreePaths.set(nodeId, new Set());
    }
    const set = this.expandedTreePaths.get(nodeId)!;
    set.has(path) ? set.delete(path) : set.add(path);
  }

  isTreeNodeExpanded(nodeId: number, path: string): boolean {
    return this.expandedTreePaths.get(nodeId)?.has(path) ?? false;
  }
  ...
  getFlattenedResponseKeys(obj: any, prefix = ''): string[] {
    ...
    if (Array.isArray(obj)) {
      ...
      return this.getFlattenedResponseKeys(obj[0], prefix ? `${prefix}[*]` : '[*]');
    }
    const keys: string[] = [];
    for (const key of Object.keys(obj)) {
      const path = prefix ? `${prefix}.${key}` : key;
      const val = obj[key];
      if (val !== null && val !== undefined && typeof val === 'object') {
        keys.push(...this.getFlattenedResponseKeys(val, path));
      } else {
        keys.push(path);
      }
    }
  ...
  private iterableDiffer = this.iterableDiffers.find([]).create<ColumnNameDatatypeName>((index, item) => item.willInclude);
      ...
      next: (response: APIResponse<ExcelRule[]>) => {
        if (response) {
          if (response.responseCode === 200) {
            ...
            for (const x of newExcelFileRule) {
  ...
  findAllIrrelevant() {
    const currentColumns = this.columnDatatype.map(col => ({
    ...
    rules.forEach(rule => {
      if (rule.ruleTypeId != RuleTypeNames.Custom && rule.ruleTypeId !== RuleTypeNames.BEValidation) {
        const ruleColumns = Array.isArray(rule.ruleColumnName)
          ? rule.ruleColumnName
          : [String(rule.ruleColumnName)];

        const hasMatch = ruleColumns.some((columnName) => {
          const currentColumn = currentColumns.find(col => col.columnName === columnName);
          if (!currentColumn) {
            return false;
          }
          return currentColumn.dbColumnName === columnName;
        });

        if (!hasMatch) {
          rule.isIrrelevant = true;
        } else {
          rule.isIrrelevant = false;
        }
  ...
  invalidTabClass(tabName: string): any {
    ...
    else if (tabName === 'apiSource-NodeList-tab') {
  ...
  onSubmit() {
    ...
    if (this.dataSource === DataSourceType.APISource) {
      //check if api source node list has a checked node
      if (!this.selectedNodeIds || this.selectedNodeIds.size === 0) {
        this.toastr.error(ModalMessages.APISourceNodeNotSelected);
        return;
      }
    }
    ...
    this.dsService.getRegion2().subscribe({
      next: (response) => {
        if (response && response.responseMessage[0] == "Success") {
          ...
          const selectedRegionId = clientInfo.get('RegionId')?.value;
          if (selectedRegionId) {
            const regionExists = this.dsRegion.some(r => String(r.id) === String(selectedRegionId));
            if (regionExists) {
              this.getSubRegion(String(selectedRegionId));
            }
          }
    ...
    this.dsService.getClients(regionId, subRegionId).subscribe({
      next: (response) => {
        if (response && response.responseMessage[0] == "Success") {
          ...
          const selectedClientId = clientInfo.get('ClientId')?.value;
          if (selectedClientId) {
            const clientExists = this.dsClient.some(c => String(c.id) === String(selectedClientId));
            if (!clientExists) {
              clientInfo.get('ClientId')?.setValue('');
            }
          }
    ...
    this.dsService.getSubRegions(regionId).subscribe({
      next: (response) => {
        if (response && response.responseMessage[0] == "Success") {
          ...
          const selectedSubRegionId = clientInfo.get('SubRegionId')?.value;
          if (selectedSubRegionId) {
            const subRegionExists = this.dsSubRegion.some(r => String(r.id) === String(selectedSubRegionId));
            if (subRegionExists) {
              this.getClient(String(regionId), String(selectedSubRegionId));
            } else {
              clientInfo.get('SubRegionId')?.setValue('');
              clientInfo.get('ClientId')?.setValue('');
            }
  ...
  onStorageAccountChange(storageAccountId: number) {
    ...
    this.formValues.deltaStorageAccountIdDisplayName = this.storageAccount.find(s => s.storageAccountId === +storageAccountId)?.storageAccountName || '';
  ...
  checkRuleChange(existingRuleId: number) {
    ...
    if (typeof columnNames !== 'string') {
      dbColumnNames = columnNames?.map((col: string) => this.columnDatatype.find(x => x.ColumnName === col)?.DbColumnName ?? col);
    } else {
      dbColumnNames = [this.columnDatatype.find(x => x.ColumnName === columnNames)?.DbColumnName ?? columnNames];
    }
    console.log(dbColumnNames);
    let columnNames2 = this.processConfigurationForm.get('ruleColumnName2')?.value;
    let dbColumnNames2 = typeof columnNames2 === 'string' && columnNames2.trim().length > 0
      ? this.columnDatatype.find(x => x.ColumnName === columnNames2)?.DbColumnName ?? columnNames2
      : '';
    console.log(dbColumnNames2);
    ...
    let aiPromptUsingDBColumns: string = '';
    ...
    switch (rule.ruleTypeId) {
      ...
      case RuleTypeNames.Custom: //custom
        ...
        const aiPromptValue = this.processConfigurationForm.get('aiPrompt')?.value ?? '';
        ruleDescription = aiPromptValue.replace(/\B@(?=\w)/g, '').trim();
        aiPromptUsingDBColumns = aiPromptValue.replace(/@(\w+)/g, (_match: string, columnName: string) => {
          const mappedDbColumnName = this.columnDatatype.find(x => x.ColumnName === columnName)?.DbColumnName;
          if (!mappedDbColumnName) {
            return `@${columnName}`;
          }
          return `@${mappedDbColumnName}`;
        }).trim();
        console.log(aiPromptUsingDBColumns);
        break;
    ...
    if (existingRuleIndex !== -1) {
      ...
      rules[existingRuleIndex] = {
        ...
        ruleDescription: ruleDescription,
        format: conditionalValue,
        prompt: aiPrompt ?? '', //aiPrompt || aiPrompt === undefined ? '' : aiPrompt,
        patternId: patternId === '' || patternId === null ? null : Number(patternId),
        conditionId: conditionId === '' || conditionId === null ? null : Number(conditionId),
        fromValue: fromValue === '' || fromValue === null ? null : Number(fromValue),
        toValue: toValue === '' || toValue === null ? null : Number(toValue),
        ...
        spNameId: spNameId,
        isUpdated: false,
        promptUsingDBColumnName: aiPromptUsingDBColumns,
        ruleDbColumnNames: dbColumnNames,
        ruleDbColumnNames2: dbColumnNames2
      }
      ...
      // Remove the previous temp copy of the same rule id before keeping the updated one.
      const tempRuleIndex = this.tmp_newlyAddedRules.findIndex(r => r.id === currentRule.id);
      if (tempRuleIndex >= 0) {
        this.tmp_newlyAddedRules.splice(tempRuleIndex, 1);
      }

      this.tmp_newlyAddedRules.push(rules[existingRuleIndex]);
  ...
  clearRule() {
    ...
    //reset the availableColumns1 and availableColumns2
    this.availableColumns1 = [...this.columnDatatype.filter(x => x.willInclude)];
    this.availableColumns2 = [...this.columnDatatype.filter(x => x.willInclude)];
  ...
  suggestions: { columnName: string; dbColumnName: string }[] = [];
  onCustomColumnNameChange(event: Event): void {
    ...
    if (match) {
      ...
      this.suggestions = (this.columnDatatype ?? []).filter(col =>
        col.ColumnName.toLowerCase().startsWith(query)
      ).map(col => ({ columnName: col.ColumnName, dbColumnName: col.DbColumnName }));
  ...
  selectSuggestion(suggestion: { columnName: string; dbColumnName: string }): void {
    const textArea = this.txtareaAiPromptRef?.nativeElement;
    if (!textArea) return;
    ...
    const updatedBefore = beforeCaret.substring(0, atIndex) + '@' + suggestion.columnName + ' ';
  ...
  confirmRegexIndex: number | null = null;
  addOrUpdateRegex() {
  ...
  selectForEdit(index: number): void {
    ...
    const ref = this.modalService.showCustomRegexBuilder(this.regexList[index]);
  ...
  askRemove(index: number, event?: MouseEvent): void {
    event?.stopPropagation();
    this.confirmRegexIndex = index;
  }

  cancelRemove(event?: MouseEvent): void {
    event?.stopPropagation();
    this.confirmRegexIndex = null;
  }
  ...
  openCustomizeTableNames() {
    const ref = this.modalService.showCustomizeTableNames(
      [...this.columnDatatype.map(item => ({ ...item }))],
      this.excelFileRule,
      this.englishOnlyCharacters,
      this.formValues.spanish_to_english,
      this.formValues.roman_numerals_only
    );
    const comp = ref.content as TableColumnNamesComponent;

    comp.onClose?.subscribe((result) => {
      if (result?.action === 'save') {
        this.columnDatatype = comp.columnNames;
        this.excelFileRule = comp.ruleSet;
        this.updateConfigOnly.emit([this.formValues, this.columnDatatype, this.excelFileRule, this.ruleSetNames, this.processConfigurationForm]);
        return;
      }
    });
