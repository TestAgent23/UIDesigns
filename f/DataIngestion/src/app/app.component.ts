...
export class AppComponent implements OnInit, OnDestroy {
  ...
  onUserLogin() {
    this.loginService.authenticate().subscribe(async (res: any) => {
      if (res.responseCode == 200 && res.responseMessage[0] == 'Success') {
        ...
        const groupId = await this.resolveSecurityGroupId(token, res.result?.securityGroups);
        if (groupId) {
  ...
  private async resolveSecurityGroupId(token: string, securityGroups?: any[]): Promise<string | null> {
    const selected = securityGroups?.find((g: any) => g?.userSelectedGroup == true)?.securityGroupId;
    const first = securityGroups?.[0]?.securityGroupId;
    if (selected) return selected;
    if (first) return first;

    try {
      const response = await firstValueFrom(this.loginService.getUserGroup(token));
      const fallbackSelected = response?.result?.find((g: any) => g?.userSelectedGroup == true)?.securityGroupId;
      const fallbackFirst = response?.result?.[0]?.securityGroupId;
      return fallbackSelected || fallbackFirst || null;
    } catch {
      return null;
    }
  }
