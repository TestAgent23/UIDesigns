...
export class NewProcessConfigurationComponent implements OnInit, OnDestroy {
  ...
  sharePointHierarchyFilterValue: SharePointTenantHierarchyFilter | null = null;
  // #endregion
  pageName: PageNames;
  ...
  async ngOnInit(): Promise<void> {
    ...
    const {
      ...
      weekDays,
      schedulerTypes
    } = this.route.snapshot.data['lookups'] as {
    ...
    this.initializeForm();
    this.syncSharePointHierarchyFilter();
    this.processConfigurationForm.valueChanges.subscribe(() => this.syncSharePointHierarchyFilter());
  ...
  initializeForm() {
    ...
    if (this.fabService.fabReady$) {
      ...
      this.processConfigurationForm.addControl('isExternalProject', new FormControl<boolean>(false));
      this.processConfigurationForm.addControl('campaignName', new FormControl<string | null>(null));
      this.processConfigurationForm.get('campaignName').disable();
    }
            ...
            next: (response: APIResponse<FileConfigurationDetails>) => {
              if (response) {
                if (response.responseCode === 200 && response.result) {
                  ...
                  if (this.configurationProcessType !== DataSourceType.LandingLayer) {
                    if (this.paramsId) {
                      const isSharePointEdit = isSharePointProcessType(this.configurationDetails?.processTypeId);
                      if (isSharePointEdit) {
                        const mappedDbColumns = (this.configurationDetails?.fileColumnMapping ?? [])
                          .filter((x) => !!x?.dbColumn)
                          .map((x) => x.dbColumn)
                          .join(',');
                        const fallbackDbColumns = (this.configurationDetails?.fileConfigurationDetails?.[0]?.columnNameList ?? '')
                          .split(',')
                          .map((x) => x?.trim())
                          .filter((x) => !!x)
                          .join(',');
                        ...
                        this.processConfigurationForm
                          ...
                          .setValue(mappedDbColumns || fallbackDbColumns);
                        ...
                        this.recordHeaders = [];
                        columnHeaders.split(',').filter((e) => !!e?.trim()).forEach((e, i) => {
                          const columnName = (response.result.fileConfigurationDetails[0].isHeaderProvided) ? e : e.split('=')[0];
                          let fileColumnMapping =
                            this.configurationDetails.fileColumnMapping.find(
                              (x) => x?.fileColumn === columnName
                            );

                          this.recordHeaders.push({
                            index: i,
                            ColumnName: columnName,
                            DbColumnName: fileColumnMapping?.dbColumn ?? columnName,
                            OgColumnName: columnName,
                            OgDbColumnName: fileColumnMapping?.dbColumn ?? columnName,
                            DatatypeName: fileColumnMapping?.dataType ?? 'string',
                            willInclude: true,
                            ColumnKey: false,
                            newColumn: false,
                            missingColumn: false,
  ...
  onSubmit(formValue: any) {
    ...
    if (isSharePointProcessType(formValue.processType)) {
    ...
    if (formValue.scheduledDate)
      sDate = `${formValue.scheduledDate.year}-${formValue.scheduledDate.month}-${formValue.scheduledDate.day}`;
    if (formValue.scheduledEndDate)
      eDate = `${formValue.scheduledEndDate.year}-${formValue.scheduledEndDate.month}-${formValue.scheduledEndDate.day}`;
  ...
  private syncSharePointHierarchyFilter(): void {
    if (!this.processConfigurationForm) return;
    const raw = this.processConfigurationForm.getRawValue();
    const next = tenantFilterFromClientInfo({
      RegionId: raw.RegionId,
      SubRegionId: raw.SubRegionId,
      ClientId: raw.ClientId,
      security_group: raw.security_group,
    });
    if (tenantFiltersEqual(this.sharePointHierarchyFilterValue, next)) return;
    this.sharePointHierarchyFilterValue = next;
  }
