...
export class FilePreviewComponent implements OnInit {
  ...
  private initialFilePreviewValues: FilePreviewValues | null = null;
  private initialRuleSetPayload: PayLoad | null = null;
  ...
  ngOnInit(): void {
    ...
    // Keep an untouched snapshot so Cancel can always restore original values.
    this.initialFilePreviewValues = structuredClone(this.filePreviewValues);
    this.initialRuleSetPayload = structuredClone(this.ruleSetPayload);
  ...
  onClose() {
    if (this.initialFilePreviewValues) {
      const originalFilePreviewValues = structuredClone(this.initialFilePreviewValues);
      Object.assign(this.filePreviewValues, originalFilePreviewValues);

      this.file = originalFilePreviewValues.file;
      this.headersRow = structuredClone(originalFilePreviewValues.headersRow);
      this.recordsArrayForDisplay = structuredClone(originalFilePreviewValues.recordsArrayForDisplay);
      this.recordHeaders = structuredClone(originalFilePreviewValues.recordHeaders);
      this.recordHeadersOG = structuredClone(originalFilePreviewValues.recordHeaders);
    }

    if (this.initialRuleSetPayload) {
      const originalRuleSetPayload = structuredClone(this.initialRuleSetPayload);
      Object.assign(this.ruleSetPayload, originalRuleSetPayload);
    }

    this.rulesetUpdated = false;
    this.newRuleSetPayload = null;
        ...
        this.confirmModalService
          ...
          .then((confirmed) => {
            if (confirmed === true) {
              ...
              this.newRuleSetPayload = structuredClone(this.ruleSetPayload);
  ...
  async onDBColumnNameChange(event: any, index: number) {
    ...
    // const matchingTempRules = (tempRuleSet ?? []).filter((x) => {
    //   const byColumns =
    ...
    //   this.ruleSetPayload.ruleSets = this.ruleSetPayload?.ruleSets?.filter(x => !uniqueTempRules.some((r: ExcelRule) => r.id === x.id));
    // }
  ...
  hasValidationRule(columnName: string): boolean {
    ...
    var columnHasRule = this.ruleSetPayload?.ruleSets.find(x => {
