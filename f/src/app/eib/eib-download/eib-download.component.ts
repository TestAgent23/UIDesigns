...
import { DBView, EIB, BusinessProcessFileUrlMapping, EIBConfigurationDetailsWithFileUrl } from '../../core/models/EIB/dbView';
  ...
  ngOnInit(): void {
    if (this.EIB) {
      this.eibService.GetEIBByEIBId(this.EIB.eibId).subscribe({
        next: (response: APIResponse<EIBConfigurationDetailsWithFileUrl>) => {
