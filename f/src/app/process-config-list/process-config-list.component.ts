...
import { SHAREPOINT_PROCESS_TYPE_ID, SHAREPOINT_SETTINGS_TAB_ID, isSharePointProcessType } from '../workspace-connect/integration/configuration-first.sharepoint';
// #endregion
...
export class ProcessConfigListComponent implements OnInit {
        ...
        if (response) {
          if (response.responseCode === 200) {
            ...
            if (isSharePointProcessType(this.configurationDetails.processTypeId)) {
              this.configurationService.getSharePointSourceByConfigId(configId).subscribe({
                next: (spResponse) => {
                  if (spResponse?.responseCode === 200 && spResponse.result && this.configurationDetails) {
                    const sp = spResponse.result;
                    this.configurationDetails.sharePointApplicationId = sp.sharePointApplicationId;
                    this.configurationDetails.sharePointApplicationSiteId = sp.sharePointApplicationSiteId;
                    this.configurationDetails.sharePointLibraryName = sp.sharePointLibraryName;
                    this.configurationDetails.sharePointFolderPath = sp.sharePointFolderPath;
                    this.configurationDetails.sharePointApplicationName = sp.sharePointApplicationName;
                    this.configurationDetails.sharePointSiteName = sp.sharePointSiteName;
                  }
                },
              });
            }
  ...
  editDetails(configId: string, tabName: string) {
    ...
    // #region Sharepoint Workspace - AY
    if (isSharePointProcessType(processType.processTypeId)) {
      const spTab = tabName?.trim() || SHAREPOINT_SETTINGS_TAB_ID;
      this.router.navigate(['/add-process', configId, spTab]);
      return;
    }
    // #endregion
