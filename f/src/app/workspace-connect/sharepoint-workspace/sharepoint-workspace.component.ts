...
@Component({
  ...
  imports: [
    ...
    WorkspaceConnectOrbitLoaderComponent,
    WorkspaceConnectStatusAlertsComponent,
    SelectDropDownModule,
  ],
  templateUrl: './sharepoint-workspace.component.html',
  styleUrls: ['./sharepoint-workspace.component.css', '../workspace-connect.styles.css'],
  host: {
...
export class SharepointWorkspaceComponent implements OnInit, OnChanges, OnDestroy {
  ...
  private lastHierarchyFilterKey = '';
  // #endregion
  ...
  @Output() navigateConnectors = new EventEmitter<void>();
  ...
  ngOnChanges(changes: SimpleChanges): void {
    if (changes['hierarchyFilter']) {
      const nextKey = tenantFilterSignature(this.hierarchyFilter);
      if (!changes['hierarchyFilter'].firstChange && nextKey !== this.lastHierarchyFilterKey) {
        this.lastHierarchyFilterKey = nextKey;
        void this.loadApplicationCatalog();
      } else if (changes['hierarchyFilter'].firstChange) {
        this.lastHierarchyFilterKey = nextKey;
      }
    }
  ...
  /** ngx-select-dropdown does not always emit (close) after a pick — sync our open state. */
  private finalizeWorkspaceDropdownSelection(): void {
    this.scheduleCloseWorkspaceDropdowns();
  }
  ...
  onFilePickAppSelect(raw: string): void {
    ...
    this.rebuildFilePickSiteDropdownOptions();
    this.disconnectWorkspace();
    this.finalizeWorkspaceDropdownSelection();
  ...
  get applicationTypeLabel(): string {
    if (this.isModeUser) return WC_CONNECTORS.typeFallback.userDelegated;
  ...
  private async loadApplicationCatalog(): Promise<void> {
    await this.scheduleAsync(async () => {
      const filter = resolveTenantFilter(this.hierarchyFilter);
      const result = await awaitApi(this.api.loadApplicationCatalog(filter), WC_CONNECTORS.api.loadFailed);
      if (!result.ok) {
