...
import { SP_UI } from './workspace-connect.messages';
  ...
  template: `
    <svg
      ...
      @switch (name) {
        ...
        @case ('connector') {
          <path d="M10 13a5 5 0 0 0 7.07 0l2.12-2.12a5 5 0 0 0-7.07-7.07l-1.41 1.41" />
          <path d="M14 11a5 5 0 0 0-7.07 0l-2.12 2.12a5 5 0 0 0 7.07 7.07l1.41-1.41" />
        }
        @case ('database') {
        ...
        @case ('azure') {
          ...
          <path d="M12 4l3 6H9l3-6z" />
        }
        @case ('aws') {
          <path d="M4 8h16v10a4 4 0 0 1-4 4H8a4 4 0 0 1-4-4Z" />
          <path d="M4 8l2-4h12l2 4" />
        }
        @case ('dropbox') {
          <path d="M12 4l8 5-8 5-8-5z" />
          <path d="M4 14l8 5 8-5" />
        }
        @case ('slack') {
          <path d="M8 12a2 2 0 1 1-4 0V9a2 2 0 0 1 4 0v3z" />
          <path d="M16 12a2 2 0 1 1-4 0V9a2 2 0 0 1 4 0v3z" />
          <path d="M12 8a2 2 0 1 1 0-4h3a2 2 0 0 1 0 4h-3z" />
          <path d="M12 16a2 2 0 1 1 0 4h3a2 2 0 0 1 0-4h-3z" />
        }
        @case ('help') {
          <circle cx="12" cy="12" r="10" />
...
export class WorkspaceConnectIconComponent {
...
@Component({
  ...
  standalone: true,
  imports: [CommonModule, WorkspaceConnectIconComponent],
...
export class WorkspaceConnectStatusAlertsComponent {
...
export class SharepointLogoComponent {
  readonly src = WC_IMAGES.sharepointMark;
}

@Component({
  selector: 'sp-google-drive-logo',
  standalone: true,
  template: `
    <img
      class="sp-google-drive-logo"
      [src]="src"
      alt=""
      aria-hidden="true"
      focusable="false" />
  ...
  styles: [
    `
      ...
      .sp-google-drive-logo {
        width: var(--sp-google-drive-logo-size, 28px);
        height: var(--sp-google-drive-logo-size, 28px);
        object-fit: contain;
        display: block;
      }
    `,
...
export class GoogleDriveLogoComponent {
  readonly src = WC_IMAGES.googleDrive;
}

export type WcModuleBreadcrumbId = 'home' | 'connector-sharepoint' | 'connector-register' | 'guided-register' | 'sharepoint-workspace';

export interface WcModuleBreadcrumbItem {
  id: WcModuleBreadcrumbId;
  label: string;
}

@Component({
  selector: 'sp-module-breadcrumb',
  standalone: true,
  imports: [CommonModule, WorkspaceConnectIconComponent, SharepointLogoComponent],
  template: `
    <nav class="sp-module-bc" [attr.aria-label]="ariaLabel">
      @for (item of items; track item.id; let i = $index; let last = $last) {
        @if (!last) {
          <button type="button" class="sp-module-bc__link" (click)="navigate.emit(item.id)">
            @if (i === 0) {
              <sp-icon class="sp-module-bc__root-icon" name="hub" size="sm" />
            }
            @if (item.id === 'connector-sharepoint') {
              <sp-sharepoint-logo />
            }
            <span>{{ item.label }}</span>
          </button>
          <sp-icon class="sp-module-bc__sep" name="chevron-right" size="sm" />
        } @else {
          <span class="sp-module-bc__current" aria-current="page">
            @if (item.id === 'home') {
              <sp-icon class="sp-module-bc__root-icon" name="hub" size="sm" />
            }
            @if (item.id === 'connector-sharepoint') {
            ...
            @if (item.id === 'connector-register') {
              <sp-icon name="plus" size="sm" />
            }
            {{ item.label }}
          </span>
        }
  ...
  styles: [
    `
      ...
      .sp-module-bc__link:hover {
        background: var(--sp-primary-light);
        border-color: rgba(46, 68, 177, 0.15);
      }
      .sp-module-bc__root-icon {
        color: var(--sp-primary);
      }
      .sp-module-bc__link sp-sharepoint-logo,
      .sp-module-bc__current sp-sharepoint-logo {
        --sp-sharepoint-logo-size: 16px;
      }
      .sp-module-bc__sep {
        color: var(--sp-text-3);
        flex-shrink: 0;
      }
      .sp-module-bc__current {
        display: inline-flex;
        align-items: center;
        gap: 6px;
        padding: 4px 8px;
...
export class WorkspaceConnectBreadcrumbComponent {
  @Input({ required: true }) items: WcModuleBreadcrumbItem[] = [];
  @Input() ariaLabel = 'Navigation';
  @Output() navigate = new EventEmitter<WcModuleBreadcrumbId>();
}
...
export class WorkspaceConnectTpLogoComponent {
  ...
  get logoSrc(): string {
    switch (this.resolvedFile) {
      case 'pa-big':
        return WC_IMAGES.tepPaBig;
      case 'pa-big-d':
        return WC_IMAGES.tepPaBigD;
      default:
        return WC_IMAGES.tepPa;
    }
  }
}
...
export class WorkspaceConnectOrbitLoaderComponent {
  @Input() title = '';
  @Input() hint = '';
  @Input() ariaLabel = '';
  @Input() compact = false;
