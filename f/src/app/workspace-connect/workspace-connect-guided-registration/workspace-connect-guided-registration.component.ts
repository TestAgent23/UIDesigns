...
export class WorkspaceConnectGuidedRegistrationComponent implements OnInit, OnDestroy {
  ...
  async finish(asInactive = false): Promise<void> {
    ...
    try {
      ...
      this.downloadCurlScriptFor(this.savedApplication);
      this.stepAnimationCounter++;
      this.microStepId = 'success';
      this.status.setSuccess(
        this.savedApplication.isActive === false
          ? this.m.successInactive(this.savedApplication.displayName)
          : this.m.successActive(this.savedApplication.displayName),
      );
      this.speakCurrentGuidance();
  ...
  private downloadCurlScriptFor(app: ApplicationDto): void {
    const curl = buildCurlCommands({
      apiBaseUrl: environment.apiBaseUrl,
      apiVersion: environment.apiVersion ?? '1.0',
      applicationId: app.applicationId,
      apiSecret: app.consumerSecret ?? '',
      displayName: app.displayName,
      sampleFolderPath: app.siteName,
    });
    downloadCurlScript(curl.fullText, curlScriptFilename(app.displayName || this.branding.applicationDefaultName));
  }
  ...
  private async loadApplicationCatalog(): Promise<void> {
    try {
      ...
      const preferredInternal = applicationTypeById(this.applicationTypes, APPLICATION_TYPE_ID.internal);
