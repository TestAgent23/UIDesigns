...
export class RegexBuilderComponent implements OnInit {
  ...
  existingRegex: RegexItem | null = null;
  ...
  ngOnInit(): void {
    ...
    if (this.existingRegex) {
      try {
        ...
        this.mode = this.existingRegex.mode == 'advanced' ? 'advanced' : 'basic';
        if (this.mode === 'advanced') {
          this.regexError = '';
          this.generatedRegex = this.existingRegex.regex;
          this.generatedDescription = this.existingRegex.description;
          this.form.get('advancedRegex')?.setValue(this.existingRegex.regex);
          this.form.get('advancedRegexDescription')?.setValue(this.existingRegex.description);
        } else {
          this.populateBasicFormFromRegex(this.existingRegex.regex);
          this.generateRegex();
        }
  ...
  generateRegex() {
    ...
    if (!this.hasBaseRule) {
      this.regexError = 'Select at least one character or pattern rule.';
      this.generatedRegex = '';
  ...
  test() {
    if (this.mode === 'advanced') {
      if (!this.form.get('advancedRegex')?.value) {
        this.testResult = '';
        return;
      }
      this.generatedRegex = this.form.get('advancedRegex')?.value;
    } else {
    ...
    //test only if filename is not blank
  ...
  onAdvancedRegexChange() {
    const regex = this.form.get('advancedRegex')?.value;
    this.regexError = '';
    if (!this.isValidRegex(regex)) {
      this.regexError = 'The provided regular expression is not valid. Please correct it before saving.';
      return;
    }
  ...
  onAdvancedRegexDescriptionChange(){
    this.generatedDescription = this.form.get('advancedRegexDescription')?.value || '';
  }
