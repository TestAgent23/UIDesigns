            ...
            for (let i = 0; i < sheetNodes.length; i++) {
              ...
              for (const match of cellMatches) {
                ...
                if (type === 's') {
                  ...
                  const num = parseFloat(rawValue);
                  if (!isNaN(num) && styleId !== null) {
                    value = getFormattedDate(num, styleId, cellXfs, customFormats,sheetName);
                    cellType = 'd';
                  } else {
                    value = rawValue;
                    cellType = 'n';
...
export function getFormattedDate(serial: number, styleId: number, stylesXml: any, customFormats: any,sheetName:string=null) {
  ...
  // FIX: decode HTML entities including currency symbols
  fmtStr = fmtStr
    .replace(/&quot;/g, '"')
    .replace(/&#34;/g, '"')
    .replace(/&#36;/g, '$')
  ...
  // Handle pipe-separated format sections (positive|negative|zero|text)
  // Excel format strings can have up to 4 sections separated by |
  // We'll use the first section for positive numbers/dates
  if (fmtStr.includes('|')) {
    const sections = fmtStr.split('|').filter(s => s.trim());
    // Use the first non-empty section
    fmtStr = sections[0] || 'general';
  }
  ...
  var value = "";
  try {
    value = XLSX.SSF.format(fmtStr || "general", serial);
  } catch (e) {
    // If formatting fails, try with "general" format as fallback
    try {
      value = XLSX.SSF.format("general", serial);
    } catch (e2) {
      // If even "general" fails, just return the raw serial number
      value = String(serial);
    }
  }
