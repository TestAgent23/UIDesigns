...
export class CreateRuleComponent implements OnInit {
  ...
  @Input() recordHeaders: ColumnNameDatatypeNameForOfflineMode[] = [];
  PageNames = PageNames;
  ...
  availableColumns1: { columnName: string, dbColumnName: string }[] = [];
  availableColumns2: { columnName: string, dbColumnName: string }[] = [];
  ...
  initializeForm() {
    ...
    if (this.column_name_list?.trim().length > 0) {
      ...
      // this.availableColumns1 = this.column_name_list.split(',').map(col => { return { columnName: col, dbColumnName: col } });
      // this.availableColumns2 = this.column_name_list.split(',').map(col => { return { columnName: col, dbColumnName: col } });
      this.availableColumns1 = this.recordHeaders.map(col => { return { columnName: col.ColumnName, dbColumnName: col.DbColumnName } });
      this.availableColumns2 = this.recordHeaders.map(col => { return { columnName: col.ColumnName, dbColumnName: col.DbColumnName } });
    }
  ...
  checkRuleChange(existingRuleId: number) {
    ...
    let dbColumnNames: any;
    let dbColumnNames2: any;
    if (ruleType !== 6) {
    ...
    if (this.pageName === PageNames.Generic_RuleList) {
      if (ruleType !== RuleTypeNames.BEValidation && ruleType !== RuleTypeNames.Custom) {
        ...
        dbColumnNames = columnNames;
        dbColumnNames2 = columnNames2;
      } else {
        columnNames = [''];
      }
    } else {
      ...
      columnNames2 = tempColumnNames2;
      if (ruleType !== RuleTypeNames.Custom) {
        if (typeof columnNames === 'string') {
          dbColumnNames = [columnNames];
        } else {
          dbColumnNames = columnNames?.map((col: string) => this.recordHeaders.find(x => x.ColumnName === col)?.DbColumnName ?? col);
        }
        console.log(dbColumnNames);

        dbColumnNames2 = typeof columnNames2 === 'string' && columnNames2.trim().length > 0
          ? this.recordHeaders.find(x => x.ColumnName === columnNames2)?.DbColumnName ?? columnNames2
          : '';
    ...
    switch (rule.ruleTypeId) {
      ...
      case RuleTypeNames.Custom: //custom
        ...
        const aiPromptValue = this.ruleCreationFormGroup.get('aiPrompt')?.value ?? '';
        ...
        aiPromptUsingDBColumns = aiPromptValue.replace(/@(\w+)/g, (_match: string, columnName: string) => {
          const mappedDbColumnName = this.recordHeaders.find(x => x.ColumnName === columnName)?.DbColumnName;
    ...
    if (existingRuleIndex !== -1) {
      ...
      this.tmp_newlyAddedRules.push(tmpRule);
  ...
  findAllIrrelevant() {
    let hasMatch: any;
    const currentColumns = this.recordHeaders.map(col => ({
    ...
    this.excelFileRule.forEach(rule => {
      if (rule.ruleTypeId != RuleTypeNames.Custom && rule.ruleTypeId != RuleTypeNames.BEValidation) {
        ...
        const columnNames = currentColumns.map(col => col.columnName);
  ...
  onCustomColumnNameChange(event: Event): void {
    ...
    if (match) {
      ...
      this.suggestions = (this.recordHeaders ?? []).filter(col =>
