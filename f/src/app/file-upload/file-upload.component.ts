...
import { MODULE_BRANDING, SP_INTEGRATION } from '../workspace-connect/core/workspace-connect.messages';
// #endregion
type AOA = any[][];
type Order = 'asc' | 'desc';
...
export class FileUploadComponent implements OnInit, AfterViewInit, AfterViewChecked {
  ...
  /** Process Settings drawer: visible from first visit until submission (local or SharePoint). */
  get showProcessSettingsPanel(): boolean {
    return !this.fileSubmissionCompleted && (this.showSharepoint || !this.isFileUploaded || this.isNewProcess);
  }
