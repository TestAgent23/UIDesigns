...
namespace Teleperformance.DataIngestion.API.Controllers.v4._1
{
    ...
    public class ProcessConfigurationController : ControllerBase
    {
        ...
        [Route("insertNewAPISource")]
        [HttpPost]
        public async Task<IActionResult> InsertNewAPISource([FromBody] InsertNewApiSourceRequest request)
        {
            _logger.LogInformation("insertNewAPISource API Called!");
            if (!ModelState.IsValid)
            {
                return BadRequest(ModelState); // This will help you debug
            }

            var result = await _processConfigurationService.InsertNewAPISource(request);
        ...
        [Route("GetWorkspaceConnectAccess")]
        [HttpGet]
        public async Task<IActionResult> GetWorkspaceConnectAccess()
        { var result = await _processConfigurationService.GetWorkspaceConnectAccess(); return StatusCode(result.ResponseCode, result); }
        #endregion
        ...
        [HttpDelete("DeletedTempFileFromLocation")]
        public async Task<IActionResult> DeletedTempFileFromLocation()
        {
            //Task<APIResponse<string>> DeletedTempFileFromLocation()
            _logger.LogInformation("GetTempFileDetails endpoint started");
            var response = await _processConfigurationService.DeletedTempFileFromLocation();
            _logger.LogInformation("GetTempFileDetails endpoint ended");
