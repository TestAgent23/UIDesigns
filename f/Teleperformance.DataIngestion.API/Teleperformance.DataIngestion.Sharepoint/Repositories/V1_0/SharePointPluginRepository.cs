...
public class SharePointPluginRepository : ISharePointPluginRepository
{
    ...
    {
        // Prefer env var (set by Program.cs in Development); Key Vault for deployed environments.
    ...
    {
        var fromEnv = WorkspaceConnectSharePointConfiguration.GetOptional(WorkspaceConnectSharePointConfiguration.ConnectionStringEnvironmentVariable);
        if (!string.IsNullOrWhiteSpace(fromEnv)) return fromEnv;

        var secretNames = new[] { WorkspaceConnectSharePointConfiguration.DataIngestionConnectionStringSecret, WorkspaceConnectSharePointConfiguration.DataIngestionV2ConnectionStringSecret };

        foreach (var secretName in secretNames)
        {
            try
            {
                var secretValue = KeyVault.GetKeyVaultValue(secretName).Result;
    ...
    public async Task<IReadOnlyList<Application>> GetApplicationsAsync(string? ownerKey, int? typeId, int? regionIdent, string? subRegionCode, int? clientIdent, Guid? securityGroupId, bool includeInactive = false)
    {
        ...
        => DbExecutor.ExecuteAsync(_connectionString, _commandTimeout, "commit_application",
            connection => connection.QuerySingleAsync<Application>("commit_application",
                new
                {
                    ...
                    Regionident = application.RegionIdent,
                    Regionname = application.RegionName,
                    Subregioncode = application.SubRegionCode,
                    Subregionname = application.SubRegionName,
                    Clientident = application.ClientIdent,
                    Clientname = application.ClientName,
                    Securitygroupid = application.SecurityGroupId,
                    Securitygroupname = application.SecurityGroupName,
                    Isactive = (application.IsActive ?? true) ? 1 : 0,
                    Isdraft = (application.IsDraft ?? false) ? 1 : 0
                },
                commandType: CommandType.StoredProcedure, commandTimeout: _commandTimeout));
    ...
    {
        var payload = sites.Select((s, index) => new { hostName = s.HostName, siteName = s.SiteName, libraryName = s.LibraryName, folderPath = s.FolderPath, sortOrder = index });
