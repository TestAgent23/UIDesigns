...
public class AzureService : IAzureService
{
    ...
    private static string GraphUrl => WorkspaceConnectSharePointConfiguration.GraphBaseUrl;
    private static string UserSelect => WorkspaceConnectSharePointConfiguration.GraphUserFields;
    ...
    private async Task<string> GetGraphTokenAsync()
    {
        ...
        try
        {
            if (!string.IsNullOrEmpty(_cachedToken) && DateTime.UtcNow < _tokenExpiry.AddMinutes(-5))
                return _cachedToken;
            // Prefer sharepointinternal* to avoid duplicated credential sets.
            var clientId = WorkspaceConnectSharePointConfiguration.GetOptional(WorkspaceConnectSharePointConfiguration.InternalClientIdEnvironmentVariable) ?? WorkspaceConnectSharePointConfiguration.GetOptional(WorkspaceConnectSharePointConfiguration.LegacyGraphClientIdEnvironmentVariable) ?? "";
            var clientSecret = WorkspaceConnectSharePointConfiguration.GetOptional(WorkspaceConnectSharePointConfiguration.InternalClientSecretEnvironmentVariable) ?? WorkspaceConnectSharePointConfiguration.GetOptional(WorkspaceConnectSharePointConfiguration.LegacyGraphClientSecretEnvironmentVariable) ?? "";
            var tenantId = WorkspaceConnectSharePointConfiguration.GetOptional(WorkspaceConnectSharePointConfiguration.InternalTenantIdEnvironmentVariable) ?? WorkspaceConnectSharePointConfiguration.GetOptional(WorkspaceConnectSharePointConfiguration.LegacyGraphTenantIdEnvironmentVariable) ?? "";
            var content = new FormUrlEncodedContent(new Dictionary<string, string> { ["client_id"] = clientId, ["client_secret"] = clientSecret, ["scope"] = WorkspaceConnectSharePointConfiguration.Get(WorkspaceConnectSharePointConfiguration.LegacyGraphScopeEnvironmentVariable, WorkspaceConnectSharePointConfiguration.DefaultGraphScope), ["grant_type"] = WorkspaceConnectSharePointConfiguration.DefaultGraphGrantType });
            var tokenBaseUrl = WorkspaceConnectSharePointConfiguration.GraphTokenBaseUrl;
    ...
    public async Task<ADUserSearchResponse> SearchADUsersAsync(string term, string? searchType = null)
    {
        ...
        catch (Exception ex) { throw new Exception(string.Format(GraphApiSearchError, ex.Message), ex); }
    }
