...
public sealed class SharePointUserContextService : ISharePointUserContextService
{
    ...
    public async Task<UserConnectSiteResult> ConnectSiteAsync(UserConnectSiteRequest request, string graphAccessToken, CancellationToken cancellationToken = default)
    {
        ...
        var (siteTitle, libraries) = await ListLibrariesViaSitesApiAsync(hostName, siteSlug, graphAccessToken, cancellationToken).ConfigureAwait(false);

        MeDrivesDiscoveryReport? discoveryReport = null;
        if (libraries.Count == 0)
        {
            (discoveryReport, libraries) = await DiscoverSiteLibrariesFromMeDrivesAsync(request, siteSlug, hostName, graphAccessToken, cancellationToken).ConfigureAwait(false);
            LogMeDrivesDiscoveryReport(discoveryReport);

            if (libraries.Count == 0)
            {
                _logger.LogInformation("UserConnectSite: /me/drives had no site libraries for siteSlug={SiteSlug}. Trying share-root fallback.", siteSlug);
        ...
        if (libraries.Count == 0) throw new UserConnectSiteFailedException(UserContextConstants.SiteLibraryNotFound, discoveryReport);

        return new UserConnectSiteResult { SiteTitle = siteTitle ?? FormatSiteTitle(siteSlug), SiteSlug = siteSlug, HostName = hostName, Libraries = libraries };
    }

    public async Task<MeDrivesDiscoveryReport> GetMeDrivesDiscoveryReportAsync(UserConnectSiteRequest request, string graphAccessToken, CancellationToken cancellationToken = default)
    {
        ArgumentNullException.ThrowIfNull(request);
        var siteSlug = UserSiteUrlBuilder.ResolveSiteSlug(request.SiteUrl, request.SiteName, request.HostName);
        var hostName = ResolveHostName(request);
        var (report, _) = await DiscoverSiteLibrariesFromMeDrivesAsync(request, siteSlug, hostName, graphAccessToken, cancellationToken).ConfigureAwait(false);
    ...
    {
        if (string.IsNullOrWhiteSpace(request.DriveId)) throw new ArgumentException(UserContextConstants.BrowseTargetRequired);

        var folderPath = NormalizeFolderPath(request.FolderPath);
        var url = string.IsNullOrEmpty(folderPath) ? _graph.BuildUrl($"drives/{request.DriveId.Trim()}/root/children") : _graph.BuildUrl($"drives/{request.DriveId.Trim()}/root:/{CommonUtilities.EncodeGraphPath(folderPath)}:/children");
    ...
    {
        if (string.IsNullOrWhiteSpace(request.DriveId) || string.IsNullOrWhiteSpace(request.FilePath)) throw new ArgumentException(UserContextConstants.FileTargetRequired);
        ...
        if (!string.IsNullOrWhiteSpace(rangeHeader)) httpRequest.Headers.TryAddWithoutValidation("Range", rangeHeader);
        ...
        return new FileStreamContent { Content = await response.Content.ReadAsStreamAsync(cancellationToken).ConfigureAwait(false), ContentType = response.Content.Headers.ContentType?.ToString() ?? "application/octet-stream", ContentLength = response.Content.Headers.ContentLength, StatusCode = (int)response.StatusCode, ContentRange = response.Content.Headers.ContentRange?.ToString(), AcceptRanges = response.Headers.AcceptRanges.Count > 0 ? string.Join(", ", response.Headers.AcceptRanges) : "bytes", FileName = CommonUtilities.ExtractFileNameFromPath(filePath) };
    }

    private async Task<(string? SiteTitle, IReadOnlyList<SharePointLibrary> Libraries)> ListLibrariesViaSitesApiAsync(string hostName, string siteSlug, string graphAccessToken, CancellationToken cancellationToken)
    {
        ...
        var siteTitle = CommonUtilities.JsonGetStringOrNull(siteRoot, "displayName") ?? CommonUtilities.JsonGetStringOrNull(siteRoot, "name");
        ...
        if (!drivesDoc.RootElement.TryGetProperty("value", out var values)) return (siteTitle, []);

        var libraries = values.EnumerateArray().Select(CommonUtilities.MapSharePointLibrary).Where(l => l is not null).Cast<SharePointLibrary>().ToList();

        return (siteTitle, OrderLibraries(libraries));
    }

    private async Task<(MeDrivesDiscoveryReport Report, IReadOnlyList<SharePointLibrary> Libraries)> DiscoverSiteLibrariesFromMeDrivesAsync(UserConnectSiteRequest request, string siteSlug, string hostName, string graphAccessToken, CancellationToken cancellationToken)
    {
        ...
        while (!string.IsNullOrWhiteSpace(nextUrl))
        {
            ...
            if (!doc.RootElement.TryGetProperty("value", out var drives)) break;
            ...
            foreach (var drive in drives.EnumerateArray())
            {
                ...
                var entry = new MeDrivesDiscoveryEntry { Index = index, Name = name, Id = id, WebUrl = webUrl, DriveType = driveType, PassesDocumentLibraryFilter = passesDocLibFilter, StrictSiteAndHostMatch = strict, SiteSlugPathMatch = slugOnly, MatchResult = DescribeDriveMatchResult(passesDocLibFilter, driveType, webUrl, siteSlug, hostName, strict, slugOnly) };
                report.Drives.Add(entry);

                if (!passesDocLibFilter) continue;

                var library = CommonUtilities.MapSharePointLibrary(drive);
                if (library is null) continue;

                if (strict) strictMatch.Add(library);
                else if (slugOnly) siteSlugMatch.Add(library);
            }

            nextUrl = doc.RootElement.TryGetProperty("@odata.nextLink", out var link) ? link.GetString() : null;
        }
    ...
    private void LogMeDrivesDiscoveryReport(MeDrivesDiscoveryReport report)
    {
        ...
        {
            log.AppendLine($"Drive #{drive.Index}: name={drive.Name ?? "(null)"} id={drive.Id ?? "(null)"} driveType={drive.DriveType ?? "(null)"}");
    ...
    private static string DescribeDriveMatchResult(bool passesDocLibFilter, string? driveType, string? webUrl, string siteSlug, string hostName, bool strict, bool slugOnly)
    {
        if (!passesDocLibFilter) return $"Excluded by driveType filter (driveType='{driveType ?? "(null)"}'; personal OneDrive drives are skipped).";

        if (string.IsNullOrWhiteSpace(webUrl)) return "Excluded: webUrl is null or empty (cannot match site path).";

        var marker = $"/sites/{siteSlug}";
        if (!SitePathContainsSlug(webUrl, siteSlug)) return $"Excluded: webUrl does not contain '{marker}'.";

        if (strict) return $"Included (strict): webUrl contains '{marker}' and host '{hostName}' or '.sharepoint.com'.";

        if (slugOnly) return $"Included (slug-only): webUrl contains '{marker}' but strict host rule failed (host expected '{hostName}').";

        return "Excluded: unexpected filter state.";
    }

    private async Task<IReadOnlyList<SharePointLibrary>> DiscoverLibrariesViaShareRootsAsync(UserConnectSiteRequest request, string siteSlug, string hostName, string graphAccessToken, CancellationToken cancellationToken)
    {
        ...
        foreach (var libraryName in new[] { "Documents", "Shared Documents" })
        {
            try
            {
                ...
                _logger.LogInformation("UserConnectSite share fallback: libraryName={LibraryName} shareUrl={ShareUrl}", libraryName, shareUrl);
                using var itemDoc = await TrySendGraphGetAsync(_graph.BuildUrl($"shares/{shareToken}/driveItem"), graphAccessToken, cancellationToken, prefer: "redeemSharingLink").ConfigureAwait(false);

                if (itemDoc is null)
                {
                    _logger.LogInformation("UserConnectSite share fallback: Graph returned non-success for libraryName={LibraryName}", libraryName);
                    continue;
                }

                var root = itemDoc.RootElement;
                if (!root.TryGetProperty("parentReference", out var parentRef)) continue;

                var driveId = CommonUtilities.JsonGetStringOrNull(parentRef, "driveId");
                if (string.IsNullOrWhiteSpace(driveId) || byDriveId.ContainsKey(driveId)) continue;

                using var driveDoc = await TrySendGraphGetAsync(_graph.BuildUrl($"drives/{driveId}"), graphAccessToken, cancellationToken).ConfigureAwait(false);

                if (driveDoc is null) continue;

                var driveEl = driveDoc.RootElement;
                var driveWebUrl = CommonUtilities.JsonGetStringOrNull(driveEl, "webUrl");
                if (!string.IsNullOrWhiteSpace(driveWebUrl) && !SiteMatchesDriveWebUrl(driveWebUrl, siteSlug, hostName)) continue;

                byDriveId[driveId] = new SharePointLibrary { Id = driveId, Name = CommonUtilities.JsonGetStringOrNull(driveEl, "name") ?? CommonUtilities.JsonGetStringOrNull(root, "name") ?? libraryName, Description = CommonUtilities.JsonGetStringOrNull(driveEl, "description"), WebUrl = driveWebUrl ?? CommonUtilities.JsonGetStringOrNull(root, "webUrl") ?? shareUrl };
            }
    ...
    {
        if (!SitePathContainsSlug(webUrl, siteSlug)) return false;

        if (string.IsNullOrWhiteSpace(hostName)) return true;

        if (webUrl.IndexOf(hostName, StringComparison.OrdinalIgnoreCase) >= 0) return true;
    ...
    {
        if (string.IsNullOrWhiteSpace(webUrl) || string.IsNullOrWhiteSpace(siteSlug)) return false;
    ...
    {
        if (!string.IsNullOrWhiteSpace(request.HostName)) return UserSiteUrlBuilder.NormalizeSharePointHostName(request.HostName);

        if (!string.IsNullOrWhiteSpace(request.SiteUrl) && Uri.TryCreate(UserSiteUrlBuilder.NormalizeSharePointUrl(request.SiteUrl.Trim()), UriKind.Absolute, out var uri)) return uri.Host;

        throw new ArgumentException(UserContextConstants.HostNameRequired);
    }

    private static string FormatSiteTitle(string siteSlug) => siteSlug.Replace('_', ' ');
    ...
    private async Task<JsonDocument?> TrySendGraphGetAsync(string url, string graphAccessToken, CancellationToken cancellationToken, string? prefer = null)
    {
        using var request = new HttpRequestMessage(HttpMethod.Get, url);
        request.Headers.Authorization = new AuthenticationHeaderValue("Bearer", graphAccessToken);
        ...
        if (!response.IsSuccessStatusCode)
        {
            ...
            _logger.LogWarning("Graph non-success (optional call) {Status} {Url}: {Body}", (int)response.StatusCode, url, body);
            return null;
        }
    ...
    private async Task EnsureGraphSuccessAsync(HttpResponseMessage response, string requestUrl, string graphAccessToken, CancellationToken cancellationToken)
    {
        if (response.IsSuccessStatusCode) return;

        var body = await response.Content.ReadAsStringAsync(cancellationToken).ConfigureAwait(false);
        var scopes = DescribeTokenScopes(graphAccessToken);
        _logger.LogError("Graph failed {Method} {Url} Status={Status} TokenScopes={Scopes} Body={Body}", response.RequestMessage?.Method, requestUrl, (int)response.StatusCode, scopes, Truncate(body, 2000));
    ...
    private static string DescribeTokenScopes(string? bearerToken)
    {
        ...
        try
        {
            ...
            if (doc.RootElement.TryGetProperty("scp", out var scp) && scp.ValueKind == JsonValueKind.String) return scp.GetString() ?? "";
            if (doc.RootElement.TryGetProperty("roles", out var roles) && roles.ValueKind == JsonValueKind.String) return roles.GetString() ?? "";
        }
        catch
        {
