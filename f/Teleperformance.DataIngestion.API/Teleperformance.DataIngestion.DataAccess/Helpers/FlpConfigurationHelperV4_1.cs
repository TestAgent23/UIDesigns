...
namespace Teleperformance.DataIngestion.DataAccess.Helpers
{
    public class FlpConfigurationHelperV4_1
    {
        ...
        public static (int chunkSize,int headerRows,int startingRows,int lastRows,List<List<object>> columns,
            List<List<object>> duplicateRowList, Dictionary<string, List<string>> uniqueRows, Dictionary<string, List<List<object>>> uniqueRowsWithDedup) InitializeExcelChunkProcessingContext(int headerRowsStarted,
                    bool isHeaderProvided,
                    int lastRowNum,
                    int skipFooterRows,
                    int lastColNum)
        {
            int chunkSize = 10000;
            int headerRows = headerRowsStarted + (isHeaderProvided ? 1 : 0);
            int startingRows = headerRows;
            int lastRows = lastRowNum - skipFooterRows;
            ...
            return (chunkSize, headerRows, startingRows, lastRows, columns, duplicateRowList, uniqueRows, uniqueRowsWithDedup);
        }
    }
}
