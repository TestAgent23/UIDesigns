            ...
            try
            {
               ...
               // string apiUrl = KeyVault.GetKeyVaultValue("TPDataIngestionValidationAPIURL").Result;
                ...
                using (HttpClient client = new HttpClient())
                {
                    ...
                    try
                    {
                        response = await client.PostAsync($"files/excel/add-rowno", content);
                        _logger.LogInformation($"AddExcelRowNo: Response received with status {response.StatusCode} for {flpConfigurationId}, {uploadFileId}");
                    }
                    catch (TaskCanceledException ex)
                    {
                        _logger.LogError($"AddExcelRowNo: Request timeout after {client.Timeout.TotalSeconds}s for {flpConfigurationId}, {uploadFileId}. Exception: {ex.Message}");
                        throw;
                    }
                    catch (HttpRequestException ex)
                    {
                        _logger.LogError($"AddExcelRowNo: HTTP request failed for {flpConfigurationId}, {uploadFileId}. Exception: {ex.Message}");
                        throw;
                    }
                    // Check if the request was successful
                    if (response.IsSuccessStatusCode)
