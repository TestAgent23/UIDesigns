...
namespace Teleperformance.DataIngestion.DataAccess.Helpers
{
    public class FlpConfigurationHelper
    {
        ...
        public static string GetFileLocation(FlpConfigurationResponseDto flpConfigurationRequestDto)
        {
            ...
            {
                var sharePointFileUrl = flpConfigurationRequestDto.SharePointFileLocation?.FileUrl;
                return !string.IsNullOrWhiteSpace(sharePointFileUrl) ? sharePointFileUrl : flpConfigurationRequestDto.SourcePath ?? string.Empty;
            }
            #endregion
            return fileLocation;
        }
        ...
        #region Workspace Connect - SharePoint
        public static async Task<bool> TryPrepareSharePointFileAsync(IFileLoadingProcessConfiguration fileLoadingProcessConfiguration, FlpRequestDto request, FlpConfigurationResponseDto configuration, params string[] allowedExtensions)
        {
            var sharePointFile = request.SharePointFileLocation;
            if (sharePointFile is null || string.IsNullOrWhiteSpace(sharePointFile.FileUrl)) return false;

            configuration.SharePointFileLocation = sharePointFile;
            configuration.SourcePath = sharePointFile.FileUrl;
            configuration.UploadedFileId = sharePointFile.UploadedId;
            var candidateName = string.IsNullOrWhiteSpace(sharePointFile.FileName) ? sharePointFile.FileUrl : sharePointFile.FileName;
            configuration.UploadedFileName = Path.GetFileName(candidateName) ?? string.Empty;

            foreach (var extension in allowedExtensions)
            {
                if (await fileLoadingProcessConfiguration.IsValidFile(configuration.FlpConfigurationId, candidateName, extension)) return true;
            }
       ...
       BlobClient csvBlobClient,ISMBLibraryServices ismbLibraryServices)
        ...
        public static string GetUploadedFileId(FlpRequestDto flpRequestDto)
        {
            ...
            else if (flpRequestDto != null && flpRequestDto.SharePointFileLocation != null)
            {
                uploadedFileId = flpRequestDto?.SharePointFileLocation?.UploadedId ?? "";
            }
            else
            {
