...
{
    public class ExcelToParquetService : IExcelToParquetService
    {
        private readonly ILogger<ExcelToParquetService> _logger;
        ...
        public ExcelToParquetService(
           ILogger<ExcelToParquetService> logger, ISMBLibraryServices ismbLibraryServices, IProcessConfigurationRepository iIProcessConfigurationRepository, ICache cache, IValidateSchemaService iValidateSchemaService, IFlpProcessingService iIFileProcessingService, IProcessConfigServiceV3 iProcessConfigurationService, IProcessConfigurationRepositoryV4_1 processConfigurationRepositoryV4_1, ICsvToParquetServiceV4_1 icsvToParquetService)
        {
            _logger = logger;
                ...
                using (var excelStream = await csvBlobClient.OpenReadAsync())
                {
                    ...
                    using (var parquetStream = await parquetBlobClient.OpenWriteAsync(overwrite: true))
                    {
                        ...
                        else
                        {
                            ...
                            (bool ret, int totalRows, int duplicateRows, Dictionary<string, DataTypeDetails> dict, FlpActivityLogStatusEnum logStatusEnum) = await ConvertExcelToParquetAsync(excelStream, parquetStream, configurationTableMappingDto, flpConfigurationResponseDto, fileExtension);
                            // Reset the memory stream position to the beginning
            ...
            try
            {
                ...
                using (var csvStream = _ismbLibraryServices.GetFileStream(fileStore, csvTempPath, flpConfigurationResponseDto.FlpConfigurationId))
                {
                    ...
                    try
                    {
                        ...
                        {
                            (bool ret, int totalRows, int duplicateRows, Dictionary<string, DataTypeDetails> dict, FlpActivityLogStatusEnum logStatusEnum) = await ConvertExcelToParquetAsync(csvStream, parquetStream, configurationTableMappingDto, flpConfigurationResponseDto, fileExtension);
                            resultResponse.ParquetFileCreated = ret;
                            resultResponse.TotalRows = totalRows;
                            resultResponse.DuplicateRows = duplicateRows;
                            resultResponse.InsertedRows = 0;
        ...
        private async Task<(bool success, int recordCount, int duplicateRows, Dictionary<string, DataTypeDetails>, FlpActivityLogStatusEnum)> ConvertExcelToParquetAsync(Stream excelStream, Stream parquetStream, ConfigurationTableMappingDto parameterConfig, FlpConfigurationResponseDto flpConfigurationResponseDto, string fileExtention)
        {
            ...
            try
            {
                ...
                else
                {
                    ...
                    success = false;
                    return (success, recordCount, duplicateRows, columnDataTypeList, FlpActivityLogStatusEnum.FileSchemaValidated);
                }
                    ...
                    if (parameterConfig.IgnoreDuplicateRows)
                    {
                        ...
                        for (int row = startRow; row <= endRow; row++)
                        {
                            ...
                            if (dedupColumnsList.Any())
                            {
                                ...
                                //    //var dbColumn = fileColumnMapping[]
                            ...
                            for (int col = firstColNum; col <= lastColNum; col++)
                            {
                                ...
                                if (columnIndices.Values.Contains(col))
                                {
                                    ...
                                    if (columnsFormat.TryGetValue(columnsKey, out DataTypeDetails DataTypeDetails))
                                    {
                                        colDataType = DataTypeDetails.DataType;
                                        //dateFormat = DataTypeDetails.DataTypeFormat;
                                        // If dateFormat is missing but data type is DateTime, assign default
                                        if (string.IsNullOrEmpty(DataTypeDetails.DataTypeFormat) && colDataType == typeof(DateTime).Name && fileExtention == "xlsb")
                                        {
                                            dateFormat = "yyyy-MM-dd"; // Or your desired format
                                        }
                                        else
                                        {
                                            dateFormat = DataTypeDetails.DataTypeFormat;
            ...
            success = true;
            return (success, recordCount, duplicateRows, columnDataTypeList, FlpActivityLogStatusEnum.ConversionToParqetFileMovedToParquetLocation);
        }
