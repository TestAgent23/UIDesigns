    ...
    public class FlpProcessingService: IFlpProcessingService
    {
        ...
        public async Task<(FlpProcessTempFile?, bool)> MoveSourceFileToTemporaryDestinationAndDelete(long processId, string fileType, string fileLocation, string fileUploadedId, string backUpFileName, ConfigurationTableMappingDto configurationTableMappingDto, FlpConfigurationResponseDto flpConfigurationRequestDto, DestinationStorageAccountDto destinationStorageAccountDto, SharedLocationDestinationServerDto slDestinationServerDto)
        {
            ...
            try
            {
                ...
                {
                    (flpProcessTempFile, isCopiedFile) = await CopySharePointFileToTemporaryDestinationAsync(
                ...
                if (isCopiedFile && flpProcessTempFile != null)
                {
                    ...
                    if (flpProcessTempFile != null)
                    {
                        ...
                        #region Workspace Connect - SharePoint
                            ...
                            && flpConfigurationRequestDto.ProcessTypeId == (int)ProcessTypeEnum.SharePointWorkspace)
                        {
                            (isDeleted, responseMsg) = await MoveSharePointFileToProcessedAsync(
                                flpConfigurationRequestDto,
                                fileLocation);
                        }
                        #endregion
      ...
      FileStatusActivityEnum fileStatusActivityEnum, FlpActivityLogStatusEnum flpActivityLogStatusEnum)
        ...
        #region Workspace Connect - SharePoint
        private async Task<(FlpProcessTempFile? TempFile, bool Copied)> CopySharePointFileToTemporaryDestinationAsync(FlpConfigurationResponseDto configuration, DestinationStorageAccountDto destinationStorageAccount, string backupFileName)
        {
            if (!configuration.SharePointApplicationId.HasValue || configuration.SharePointApplicationId.Value == Guid.Empty || string.IsNullOrWhiteSpace(configuration.SourcePath))
            {
                _logger.LogError("SharePoint source configuration is incomplete for {FlpConfigurationId}", configuration.FlpConfigurationId);
                return (null, false);
            }

            var credentials = new WorkspaceCredentials { ApplicationId = configuration.SharePointApplicationId.Value, ApplicationSiteId = configuration.SharePointApplicationSiteId, LibraryName = configuration.SharePointLibraryName };

            var fileContent = await _sharePointPluginService.FetchFileAsync(credentials, configuration.SourcePath, null);
            if (fileContent.Content is null || fileContent.Content == Stream.Null) return (null, false);
            await using var sourceStream = fileContent.Content;
            if (fileContent.Content.CanSeek) fileContent.Content.Position = 0;

            var tempFile = new FlpProcessTempFile();
            var contentLength = fileContent.ContentLength ?? (fileContent.Content.CanSeek ? fileContent.Content.Length : 0);
            tempFile.FileSize = contentLength / (1024d * 1024d);

            if (configuration.DestinationLocationTypeId == (int)DestinationLocationTypeEnum.Azure)
            {
                if (destinationStorageAccount is null) return (null, false);

                var connectionString = FlpConfigurationHelper.GetBlobConnectionString(destinationStorageAccount.FlpStorageAccount, destinationStorageAccount.StorageAccountKey);
                var destinationPrefix = configuration.DestinationPath.TrimEnd('/', '\\');
                var blobName = $"{destinationPrefix}/temp/{DateTime.UtcNow:yyyyMMdd}/{backupFileName}";
                var blobClient = _iBlobStorageService.GetBlobClientDetails(blobName, connectionString, destinationStorageAccount.StorageContainerName);
                await blobClient.UploadAsync(fileContent.Content, overwrite: true);

                tempFile.Name = blobClient.Name;
                tempFile.Uri = blobClient.Uri.ToString();
                tempFile.CanGenerateSasUri = blobClient.CanGenerateSasUri;
                tempFile.AccountName = destinationStorageAccount.FlpStorageAccount;
                tempFile.BlobContainerName = destinationStorageAccount.StorageContainerName;
                tempFile.ParquetBlobConnectionString = connectionString;
                tempFile.DestinationFolder = $"{destinationPrefix}/parquet/";
                return (tempFile, true);
            }

            if (configuration.DestinationLocationTypeId == (int)DestinationLocationTypeEnum.OnPrem)
            {
                var destinationFolder = Path.Combine(configuration.DestinationPath, "temp", $"{DateTime.UtcNow:yyyyMMdd}");
                Directory.CreateDirectory(destinationFolder);
                var tempFilePath = Path.Combine(destinationFolder, backupFileName);
                await using var output = File.Create(tempFilePath);
                await fileContent.Content.CopyToAsync(output);

                tempFile.sourceTempFilePath = tempFilePath;
                tempFile.DestinationFolder = Path.Combine(configuration.DestinationPath, "parquet");
                return (tempFile, true);
            }
        ...
        private async Task<(bool Moved, string Message)> MoveSharePointFileToProcessedAsync(FlpConfigurationResponseDto configuration, string? sourceFilePath)
        {
            try
            {
                if (!configuration.SharePointApplicationId.HasValue || configuration.SharePointApplicationId.Value == Guid.Empty)
                {
                    return (false, Teleperformance.DataIngestion.Sharepoint.Constants.Constants.SharePointApplicationIdMissing);
                }

                var source = string.IsNullOrWhiteSpace(sourceFilePath) ? configuration.SourcePath : sourceFilePath;
                if (string.IsNullOrWhiteSpace(source)) return (false, Teleperformance.DataIngestion.Sharepoint.Constants.Constants.SharePointSourcePathMissing);

                var credentials = new WorkspaceCredentials { ApplicationId = configuration.SharePointApplicationId.Value, ApplicationSiteId = configuration.SharePointApplicationSiteId, LibraryName = configuration.SharePointLibraryName };
                var processedFolder = ResolveSharePointProcessedFolderPath(configuration.SharePointFolderPath, source);
                var newName = $"{Path.GetFileNameWithoutExtension(source)}_{DateTime.UtcNow:yyyyMMdd_HHmmss}{Path.GetExtension(source)}";
                var moved = await _sharePointPluginService.MoveFileAsync(credentials, source, processedFolder, newName, createDestinationIfMissing: true);
                return moved ? (true, string.Format(Teleperformance.DataIngestion.Sharepoint.Constants.Constants.MovedToProcessed, processedFolder, newName)) : (false, Teleperformance.DataIngestion.Sharepoint.Constants.Constants.MoveToProcessedFailed);
            }
            catch (Exception ex)
            {
                _logger.LogError(ex, "Error moving SharePoint source file for {FlpConfigurationId}", configuration.FlpConfigurationId);
                return (false, Teleperformance.DataIngestion.Sharepoint.Constants.Constants.MoveToProcessedError);
            }
        }
