...
namespace Teleperformance.DataIngestion.DataAccess.Services.v1._0
{
    public class ProcessConfigurationService : IProcessConfigurationService
    {
        ...
        {
            if (string.IsNullOrWhiteSpace(flpConfigurationId)) return new APIResponse<FlpSharePointSourceDetail?> { ResultStatus = APIResultStatus.InvalidParameters, ResponseMessage = new List<string> { Teleperformance.DataIngestion.Sharepoint.Constants.Constants.MissingConfigurationId }, Result = null };

            var source = await _processConfigurationRepository.GetSharePointSourceByConfigId(flpConfigurationId);
            return new APIResponse<FlpSharePointSourceDetail?> { ResultStatus = APIResultStatus.Completed, ResponseMessage = new List<string> { Teleperformance.DataIngestion.Sharepoint.Constants.Constants.SuccessMessage }, Result = source };
        }
        #endregion
