    ...
    public class FlpProcessingServiceV4 : IFlpProcessingServiceV4
    {
            ...
            try
            {
                ...
                if (isCopiedFile && flpProcessTempFile != null)
                {
                    ...
                    if (flpProcessTempFile != null)
                    {
                        ...
                        {
                            (isDeleted, responseMsg) = await MoveSharePointFileToProcessedAsync(flpConfigurationRequestDto, fileLocation);
                        }
                        #endregion
      ...
      FileStatusActivityEnum fileStatusActivityEnum, FlpActivityLogStatusEnum flpActivityLogStatusEnum,string databricksAPIResponse)
        ...
        private async Task<(FlpProcessTempFile? TempFile, bool Copied)> CopySharePointFileToTemporaryDestinationAsync(FlpConfigurationResponseDto configuration, DestinationStorageAccountDto destinationStorageAccount, string backupFileName)
        {
            ...
            var contentLength = fileContent.ContentLength ?? (sourceStream.CanSeek ? sourceStream.Length : 0);
            ...
            if (configuration.DestinationLocationTypeId == (int)DestinationLocationTypeEnum.Azure)
            {
                ...
                await blobClient.UploadAsync(sourceStream, overwrite: true);
