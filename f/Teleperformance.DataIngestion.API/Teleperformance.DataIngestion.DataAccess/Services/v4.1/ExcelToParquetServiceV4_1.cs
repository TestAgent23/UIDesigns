...
{
    public class ExcelToParquetServiceV4_1 : IExcelToParquetServiceV4_1
    {
        private readonly ILogger<ExcelToParquetServiceV4_1> _logger;
        ...
        public ExcelToParquetServiceV4_1(
           ILogger<ExcelToParquetServiceV4_1> logger, ISMBLibraryServices ismbLibraryServices, ICache cache, IValidateSchemaServiceV4_1 iValidateSchemaService, IFlpProcessingServiceV4_1 iIFileProcessingService, IProcessConfigServiceV3 iProcessConfigurationService, ICsvToParquetServiceV4_1 icsvToParquetService, IProcessConfigurationRepositoryV4_1 processConfigurationRepositoryV4_1)
        {
            _logger = logger;
        ...
        public async Task<ParquetFileResponseDtoV4_1> ConvertDataToParquet(ConfigurationTableMappingDto configurationTableMappingDto, FlpConfigurationResponseDtoV4_1 flpConfigurationResponseDto, FlpProcessTempFileModel fileProcessingConfig)
        {
            ...
            try
            {
                ...
                string parquetFilePath = string.Empty;
                if (!string.IsNullOrWhiteSpace(configurationTableMappingDto.TabName))
                {
                    fileName = configurationTableMappingDto.TabName +"_"+ FlpConfigurationHelper.GetParquetFileName(fileProcessingConfig.Name);
                    parquetFilePath = $"{fileProcessingConfig.DestinationFolder}{fileName}";
                }
                else
                {
                    fileName = FlpConfigurationHelper.GetParquetFileName(fileProcessingConfig.Name);
                    parquetFilePath = $"{fileProcessingConfig.DestinationFolder}{fileName}";
                }
                ...
                using (var excelStream = await csvBlobClient.OpenReadAsync())
                {
                    ...
                    using (var parquetStream = await parquetBlobClient.OpenWriteAsync(overwrite: true))
                    {
                        ...
                        if (fileSizeInMB > fileSize)
                        {
                            ...
                            var response = await _icsvToParquetService.ConvertCsvToParquetAsync(csvStream, parquetStream, configurationTableMappingDto, flpConfigurationResponseDto);
        ...
        private async Task<(bool success, int recordCount, int insertedRows, int duplicateRows, Dictionary<string, DataTypeDetails>, FlpActivityLogStatusEnum)> ConvertExcelToParquetAsync(Stream excelStream, Stream parquetStream, ConfigurationTableMappingDto parameterConfig, FlpConfigurationResponseDtoV4_1 flpConfigurationResponseDto, string fileExtention)
        {
            int recordCount = 0;
            int duplicateRows = 0;
            int insertedRows = 0;
           ...
           // double fileSize = 0;
            long fileSizeInBytes = excelStream.Length;
            double fileSize = fileSizeInBytes / (1024.0 * 1024.0);

            if (string.Compare(fileExtention, ".xls", true) == 0)
            {
              //workbook = new HSSFWorkbook(excelStream); // Use HSSF for .xls files
               workbook = new HSSFWorkbook(excelStream); // Use XSSF for .xlsx files
               
            }
            else if (string.Compare(fileExtention, ".xlsb", true) == 0)
            {
                xlsbWorkbookModel = ExcelHelper.ConvertXLSBtoXLSXV3(excelStream,parameterConfig.TabName,null); // Convert .xlsb to .xlsx
            ...
            var sheet = workbook.GetSheet(parameterConfig.TabName); // Get the first sheet
            ...
            {
                int skipCount = parameterConfig.SkipRows;
                ...
                //reset again headerRowsStarted to emptyHeaderRowsIndex
               // if (emptyHeaderRowsIndex > 0)
                  headerRowsStarted = emptyHeaderRowsIndex;
            }
            ...
            try
            {
                ...
                if (parameterConfig.IsHeaderProvided)
                {
                    ...
                    if (filesHeaders.Any())
                    {
                        ...
                        fileColumnMapping = await _iValidateSchemaService.ValidateFileSchema(parameterConfig.FlpConfigurationId, parameterConfig.ValidateFileSchema, parameterConfig.IsHeaderProvided, copyFielesHeaders, flpConfigurationResponseDto.ProcessName, parameterConfig.TabName, parameterConfig.TableName);
                ...
                else
                {
                    ...
                    if ((flpConfigurationResponseDto.UIValidation || flpConfigurationResponseDto.BEValidation))
                    {
                        ...
                        if (flpCustomHeaderList.Any())
                        {
                            ...
                            if (parameterConfig.SpanishToEnglish)
                            {
                                fileColumnMapping = await _iValidateSchemaService.ValidateFileSchema(parameterConfig.FlpConfigurationId, parameterConfig.ValidateFileSchema, parameterConfig.IsHeaderProvided, flpCustomHeaderList, flpConfigurationResponseDto.ProcessName, parameterConfig.TabName, parameterConfig.TableName);
                            }
                            else
                            {
                                fileColumnMapping = await _iValidateSchemaService.ValidateFileSchema(parameterConfig.FlpConfigurationId, parameterConfig.ValidateFileSchema, parameterConfig.IsHeaderProvided, flpCustomHeaderList, flpConfigurationResponseDto.ProcessName, parameterConfig.TabName, parameterConfig.TableName);
            ...
            using (var parquetWriter = await ParquetWriter.CreateAsync(schema, parquetStream))
            {
                ...
                var(chunkSize, headerRows, startingRows, lastRows, columns, duplicateRowList, uniqueRows, uniqueRowsWithDedup) = FlpConfigurationHelperV4_1.InitializeExcelChunkProcessingContext(headerRowsStarted,parameterConfig.IsHeaderProvided,
                                                            lastRowNum, parameterConfig.SkipFooterRows, lastColNum);
                                                    ...
                                                    if (cell?.CellType == CellType.Formula)
                                                    {
                                                        switch (cell.CachedFormulaResultType)
                                                        {
                                                            case CellType.Numeric:
                                                                var formulaFmtStr3 = cell.CellStyle?.GetDataFormatString() ?? string.Empty;
                                                                if (formulaFmtStr3.Contains("%"))
                                                                    value = (cell.NumericCellValue * 100).ToString("0.##", CultureInfo.InvariantCulture) + "%";
                                                                else
                                                                    value = DateUtil.IsCellDateFormatted(cell)
                                                                        ? new DataFormatter(CultureInfo.InvariantCulture).FormatCellValue(cell, workbook.GetCreationHelper().CreateFormulaEvaluator())
                                    ...
                                    for (int col = firstColNum; col <= lastColNum; col++)
                                    {
                                        ...
                                        if (cell?.CellType == CellType.Formula)
                                        {
                                            switch (cell.CachedFormulaResultType)
                                            {
                                                ...
                                                case CellType.Numeric:
                                                    var formulaFmtStr2 = cell.CellStyle?.GetDataFormatString() ?? string.Empty;
                                                    if (formulaFmtStr2.Contains("%"))
                                                        cellValue = (cell.NumericCellValue * 100).ToString("0.##", CultureInfo.InvariantCulture) + "%";
                                                    else
        ...
        private DataColumn ConvertArrayInValidDataType(DataField dataField, object[] dataArray, string format, bool dedupCol, List<string> sheetFormatList)
        {
            ...
            else if (dataField is DataField<Int64?>)
            {
                ...
                dataColumn = new DataColumn(dataField, dataArray.Select(x =>
                {
                    ...
                    // Try parsing as a decimal to support scientific notation and avoid precision loss
                    if (decimal.TryParse(valueStr, System.Globalization.NumberStyles.Float,
                                        System.Globalization.CultureInfo.InvariantCulture, out decimalValue))
                    {
                        return (long?)decimalValue; // Convert to long safely
                    }
