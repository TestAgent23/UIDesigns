...
namespace Teleperformance.DataIngestion.DataAccess.Services.v4._1
{
    public class ProcessConfigurationServiceV4_1 : IProcessConfigurationServiceV4_1
    {
                            ...
                            foreach (var fileSetting in jsonData.fileSettings)
                            {
                                ...
                                foreach (var rule in fileSetting.ruleSet)
                                {
                                    var ruleSet = new RuleSetEntity
                                    {
                                        ...
                                        ruleDbColumnNames = rule.ruleDbColumnNames,
                                        ruleDbColumnNames2 = rule.ruleDbColumnNames2,
                                        ruleDescription = rule.ruleDescription,
                                        prompt = rule.prompt,
                                        promptUsingDBColumnName = rule.promptUsingDBColumnName,
                                        format = rule.format,
        ...
        public async Task<APIResponse<List<RuleSetDto>>> GetDIRuleSetByRuleSetNameId(string ruleSetNameId)
        {
            ...
            return new APIResponse<List<RuleSetDto>>
            {
                ...
                Result = DIFlpRuleSet.Select(r => new RuleSetDto
                {
                    ...
                    ruleDbColumnNameRaw = r.ruleDbColumnNameRaw,
                    ruleDbColumnNames2 = r.ruleDbColumnNames2 ?? string.Empty,
                    promptUsingDBColumnName = r.promptUsingDBColumnName,
        ...
        public async Task<APIResponse<bool>> GetWorkspaceConnectAccess()
        {
            try
            {
                string securityGroupId = _headerService.GetHeaderValue("x-tpdi-api-sg");
                if (string.IsNullOrWhiteSpace(securityGroupId)) return new APIResponse<bool> { ResultStatus = APIResultStatus.InvalidParameters, ResponseMessage = new List<string> { Teleperformance.DataIngestion.Sharepoint.Constants.Constants.SecurityGroupNotFound }, Result = false };

                var databaseResponse = await _processConfigurationRepository.GetWorkspaceConnectAccess(securityGroupId);
                return new APIResponse<bool> { ResultStatus = APIResultStatus.Completed, ResponseMessage = new List<string> { Teleperformance.DataIngestion.Sharepoint.Constants.Constants.SuccessMessage }, Result = databaseResponse };
            }
            catch (Exception ex)
            {
                _logger.LogError(ex, ex.Message);
                return new APIResponse<bool> { ResultStatus = APIResultStatus.Failed, ResponseMessage = new List<string> { Teleperformance.DataIngestion.Sharepoint.Constants.Constants.UnableToRetrieveAccess }, Result = false };
            }
        }
        #endregion
        ...
        public async Task<APIResponse<string>> AddCampaignUserByClientGeoMapping(AddCampaignUserByClientGeoMapping request)
        {
            try
            {
                ...
                if (string.IsNullOrWhiteSpace(request.Upn))
                {
                    ...
                    {
                        ResultStatus = APIResultStatus.InvalidParameters,
                        ResponseMessage = new List<string> { "UPN cannot be empty." },
                        Result = null
                    };
                }

                // C# responsibility: split UPN only
                var upnList = request.Upn
                ...
                if (!upnList.Any())
                {
                    ...
                    {
                        ResultStatus = APIResultStatus.InvalidParameters,
                        ResponseMessage = new List<string> { "No valid UPN values found." },
                        Result = null
                    };
                }

                // Single SP call — SP handles campaign lookup + deactivate + insert
                var result = await _processConfigurationRepository.BulkUpdateCampaignUserAccessByGeoMapping(
                    request.RegionId, request.SubRegionId, request.ClientId, applicationId, upnList);

                if (string.Compare(result?.Result, "NotFound", StringComparison.OrdinalIgnoreCase) == 0)
                {
                ...
                bool isFailure = string.Compare(result?.Result, "Error", StringComparison.OrdinalIgnoreCase) == 0;

                _logger.LogInformation($"Bulk campaign user access add: {result?.RecordId} record(s) processed.");

                return new APIResponse<string>
                {
                    ResultStatus = isFailure ? APIResultStatus.Error : APIResultStatus.Completed,
                    ResponseMessage = new List<string> { isFailure ? "Failed to add user access records." : $"{result?.RecordId} user access record(s) added successfully." },
                    Result = isFailure ? "Error occurred while processing the request." : "Processed Completed successfully."
                };
            }
            catch (Exception ex)
            {
        ...
        public async Task<APIResponse<string>> RemoveCampaignUserByClientGeoMapping(AddCampaignUserByClientGeoMapping request)
        {
            try
            {
                ...
                // Single SP call — SP handles campaign lookup + deactivate
                var result = await _processConfigurationRepository.BulkDeleteCampaignUserAccessByGeoMapping(
                    request.RegionId, request.SubRegionId, request.ClientId, applicationId, upnList);
                ...
                _logger.LogInformation($"Bulk campaign user access remove: {result?.RecordId} record(s) processed.");

                return new APIResponse<string>
                {
                    ResultStatus = isFailure ? APIResultStatus.Error : APIResultStatus.Completed,
                    ResponseMessage = new List<string> { isFailure ? "Remove operation failed to remove." : "Remove operation completed successfully." },
                    Result = isFailure ? "Error occurred while processing the request." : "Processed Completed successfully."
                };
            }
            catch (Exception ex)
        ...
        public async Task<APIResponse<string>> AddCampaignSuperAdmin(AddCampaignSuperAdmin request)
        {
            try
            {
                ...
                // Single SP call — SP handles campaign lookup + deactivate + insert for ALL campaigns
                var result = await _processConfigurationRepository.BulkUpdateCampaignUserAccessSuperAdmin(applicationId, upnList);

                if (string.Compare(result?.Result, "NotFound", StringComparison.OrdinalIgnoreCase) == 0)
                {
                    _logger.LogError("Error: Not found campaign details");
                    return new APIResponse<string>
                    {
                        ResultStatus = APIResultStatus.InvalidParameters,
                        ResponseMessage = new List<string> { "Error: Not found campaign details" },
                        Result = null
                    };
                }
                ...
                _logger.LogInformation($"Bulk SuperAdmin user access add: {result?.RecordId} record(s) processed.");

                return new APIResponse<string>
                {
                    ResultStatus = isFailure ? APIResultStatus.Error : APIResultStatus.Completed,
                    ResponseMessage = new List<string> { isFailure ? $"{upnList.Count} user access record(s) failed to add." : $"{result?.RecordId} user access record(s) added successfully." },
                    Result = isFailure ? "Error occurred while processing the request." : "Processed Completed successfully."
                };
            }
            catch (Exception ex)
        ...
        public async Task<APIResponse<string>> RemoveCampaignSuperAdmin(AddCampaignSuperAdmin request)
        {
            try
            {
                ...
                // Single SP call — SP handles campaign lookup + deactivate for ALL campaigns
                var result = await _processConfigurationRepository.BulkDeleteCampaignUserAccessSuperAdmin(applicationId, upnList);
                ...
                _logger.LogInformation($"Bulk SuperAdmin user access remove: {result?.RecordId} record(s) processed.");

                return new APIResponse<string>
                {
                    ResultStatus = isFailure ? APIResultStatus.Error : APIResultStatus.Completed,
                    ResponseMessage = new List<string> { isFailure ? "Remove operation failed to delete." : "Remove operation completed successfully." },
                    Result = isFailure ? "Error occurred while processing the request." : "Processed Completed successfully."
                };
            }
            catch (Exception ex)
        ...
        //        // ✅ Thread-safe collections for parallel writes
        //        var addedUserAccessRecords = new ConcurrentBag<string>();
        //        var failedUserAccessRecords = new ConcurrentBag<string>();
        ...
        //            if (upnList.Any())
        //            {
        //                // ✅ Limit to 20 concurrent DB connections — stays well within the SQL Server connection pool limit (default 100)
        //                using var semaphore = new SemaphoreSlim(20, 20);

        //                // ✅ Flatten the nested loops into a single list of parallel tasks
        //                var tasks = existingCampaigns
        //                    .Where(c => !string.IsNullOrEmpty(c.InternalCampaignId))
        //                    .SelectMany(campaign => upnList.Select(upn => (campaign, upn)))
        //                    .Select(async pair =>
        //                    {
        ...
        //                            var campaignUserAccess = new FlpCampaignUserAccess
        //                            {
        //                                CampaignUserAccessId = Guid.NewGuid().ToString(),
        //                                InternalCampaignId = pair.campaign.InternalCampaignId,
        //                                Upn = pair.upn
        //                            };
        ...
        //                            if (string.Compare(userAccessResult.Result, "Failure", true) == 0)
        //                            {
        //                                failedUserAccessRecords.Add($"{pair.upn} for {pair.campaign.CampaignId}");
        //                                _logger.LogWarning($"Failed to update user access for UPN: {pair.upn} and Campaign: {pair.campaign.CampaignId}. Message: {userAccessResult?.Message}");
        //                            }
        //                            else
        //                            {
        //                                addedUserAccessRecords.Add($"{pair.upn} for {pair.campaign.CampaignId}");
        //                                _logger.LogInformation($"Campaign user access updated successfully for UPN: {pair.upn} and Campaign: {pair.campaign.CampaignId}");
        //                            }
        //                        }
        //                        catch (Exception innerEx)
        //                        {
        //                            failedUserAccessRecords.Add($"{pair.upn} for {pair.campaign.CampaignId}");
        //                            _logger.LogError(innerEx, $"Error updating campaign user access for UPN: {pair.upn} and Campaign: {pair.campaign.CampaignId}");
        //                        }
        //                        finally
        //                        {
        //                            semaphore.Release();
        ...
        //                // ✅ Execute all tasks in parallel — waits for all to complete
        ...
        //        string msg = failedUserAccessRecords.Any()
        ...
        //        // ✅ Thread-safe collections for parallel writes
        //        var removedUserAccessRecords = new ConcurrentBag<string>();
        //        var failedUserAccessRecords = new ConcurrentBag<string>();
        ...
        //                            else
        //                            {
        //                                removedUserAccessRecords.Add($"{pair.upn} for {pair.campaign.CampaignId}");
        //                                _logger.LogInformation($"Campaign user access Deleted successfully for UPN: {pair.upn}");
        //                            }
        //                        }
        //                        catch (Exception innerEx)
        //                        {
        //                            failedUserAccessRecords.Add($"{pair.upn} for {pair.campaign.CampaignId}");
        //                            _logger.LogError(innerEx, $"Error Deleting campaign user access for UPN: {pair.upn}");
        //                        }
        //                        finally
        //                        {
        //                            semaphore.Release();
        ...
        //        if (removedUserAccessRecords.Any())
        ...
        #region API As a source
        public async Task<APIResponse<ConfigurationAPISourceResponseDto>> InsertNewAPISource(InsertNewApiSourceRequest request)
        {
            if (string.IsNullOrEmpty(request.apiSource.exportJSON))
            {
                _logger.LogError("Error: exportJSON is null or empty.");
                return new APIResponse<ConfigurationAPISourceResponseDto>
                {
                    ResultStatus = APIResultStatus.InvalidParameters,
                    ResponseMessage = new List<string> { "exportJSON cannot be null or empty." },
                    Result = null
                };
            }
            ...
            //lets save the configuration to generate flpconfigurationid
            var processConfiguration = new ProcessSettingEntity
            {
                flpConfigurationId = request.config.processSettings.flpConfigurationId,
                process_name = request.config.processSettings.processName,
                is_active = true,// jsonData.processSettings.is_active,
                sender_communication_email = request.config.processSettings.sender_communication_email,
                created_by = request.loggedInUser,
                created_date = DateTime.UtcNow,
                loginid = request.loggedInUser,
                description = request.config.processSettings.description,
                regionId = request.config.processSettings.RegionId,
                subRegionId = request.config.processSettings.SubRegionId,
                clientId = request.config.processSettings.ClientId,
                userName = request.userName,
                securityGroupId = securityGroupId,
                securityGroups = request.config.processSettings.securityGroups,
                region = request.config.processSettings.region,
                subRegion = request.config.processSettings.subRegion,
                clientName = request.config.processSettings.clientName,
                dataSource = request.config.processSettings.dataSource,
                multisheet = request.config.processSettings.multisheet,
                sheetReferenceByIndex = request.config.processSettings.sheetReferenceByIndex,
                internalCampaignId = request.config.fileSettings[0].additionalSettings.internalCampaignId
            ...
            using (var scope = new TransactionScope(TransactionScopeAsyncFlowOption.Enabled))
            {
                try
                {
                    foreach (Models.Entities.v2._0.ProcessConfiguration.SecurityGroup securityGroup in request.config.processSettings.securityGroups)
                    {
                        var retVal = await _processConfigRepositoryV4.AddSecurityGroup(securityGroup);
                    }

                    if(string.IsNullOrEmpty(processConfiguration.flpConfigurationId))
                    {
                        processConfiguration.flpConfigurationId = Guid.NewGuid().ToString();
                        var retVal = await _processConfigurationRepository.InsertProcessConfiguration(processConfiguration);
                ...
                {
                    _logger.LogError(ex, "Error inserting NewAPISource configuration: {Message}", ex.Message);

                    return new APIResponse<ConfigurationAPISourceResponseDto>
                    {
                        ResultStatus = APIResultStatus.Failed,
                        ResponseMessage = new List<string> { "Error inserting NewAPISource configuration." },
                        Result = null
                    };
                }
            }

            // Implement the logic for inserting a new API source here
            throw new NotImplementedException();
        }
        #endregion
        ...
        public async Task<APIResponse<string>> DeletedTempFileFromLocation()
        {
            try
            {
                var databaseResponse = await _processConfigurationRepository.GetTempFileDetailsAsync();
                int fileDeletedCount = 0;
                foreach (var file in databaseResponse)
                {
                    try
                    {
                        _logger.LogInformation($"Deleting file: {file.blobName} for FlpConfigurationId: {file.flpConfigurationId}");
                        BlobClient blobClient;

                        if (!string.IsNullOrWhiteSpace(file.sasKey))
                        {
                            //var blobUri = new UriBuilder
                            //{
                            //    Scheme = "https",
                            //    Host = $"{file.storageAccountName}.blob.core.windows.net",
                            //    Path = $"{file.storageContainerName}/{file.blobName}",
                            //    Query = file.sasKey.TrimStart('?')
                            //}.Uri;
                            //blobClient = new BlobClient(blobUri);
                            var sasToken = file.sasKey.Contains("?") ? file.sasKey.Substring(file.sasKey.IndexOf('?')) : file.sasKey;
                            var blobUri = new UriBuilder
                            {
                                Scheme = "https",
                                Host = $"{file.storageAccountName}.blob.core.windows.net",
                        ...
                        {
                            _logger.LogWarning($"SAS key is missing for file: {file.blobName}. Falling back to connection string.");
                            string connectionString = FlpConfigurationHelper.GetBlobConnectionString(file.storageAccountName, file.storageAccountKey);
                            blobClient = _blobStorageService.GetBlobClientDetails(file.blobName, connectionString, file.storageContainerName);
                        }

                        var deleted = await _blobStorageService.DeleteBlobAsync(blobClient);
                        var result = await _processConfigurationRepository.UpdateFlagDeletedTempFileAsync(file.flpConfigurationId, file.uploadFileId,deleted);
                        if(result !=null  && result.Result== "Success")
                        {
                            fileDeletedCount++;
                            _logger.LogInformation($"Successfully deleted file: {file.blobName} and updated flag.");
                        }
                        else
                        {
                            _logger.LogWarning($"Failed to delete file: {file.blobName}");
                        }
                    ...
                    {
                        _logger.LogError(ex, $"An error occurred while deleting file: {file.blobName}");
                    }
                }

                if (databaseResponse.Any() && databaseResponse.Count() == fileDeletedCount)
                {
                    _logger.LogInformation("All temporary files deleted successfully.");
                    return await Task.FromResult(new APIResponse<string>
                    {
                        ResultStatus = APIResultStatus.Completed,
                        ResponseMessage = new List<string> { "All temporary files deleted successfully." },
                        Result = "Success"
                    });
                }
                else if (databaseResponse.Any())
                {
                    _logger.LogWarning("Not all temporary files were deleted.");
                    return await Task.FromResult(new APIResponse<string>
                    {
                        ResultStatus = APIResultStatus.Failed,
                        ResponseMessage = new List<string> { "Could not delete all temporary files." },
                        Result = "Failure"
                    });
                }
                else
                {
                    _logger.LogInformation("No temporary files found to delete.");
                    return await Task.FromResult(new APIResponse<string>
                    {
                        ResultStatus = APIResultStatus.Completed,
                        ResponseMessage = new List<string> { "No temporary files to delete." },
                        Result = "Success"
                    });
                }
            }
            catch (Exception ex)
            {
                ...
                {
                    ResultStatus = APIResultStatus.Failed,
                    ResponseMessage = new List<string> { $"{ex.Message.ToString()}" },
                    Result = null
                });
            }
        }
