/*
  PROD DEPLOY: dbo.sel_flpConfigurationList (DataIngestionV2)
  SharePoint-only change — no scheduler, WHERE, or other JOIN logic modified.

  Changed (4 SELECT expressions + 1 LEFT JOIN):
    - COALESCE main-table sharePoint* with di_flpSharePointSource (legacy rows keep main-table values)
    - LEFT JOIN dbo.di_flpSharePointSource sp_src ON flpConfigurationId, active = 1

  Verify after deploy:
    SELECT CASE WHEN OBJECT_DEFINITION(OBJECT_ID('dbo.sel_flpConfigurationList'))
      LIKE '%COALESCE(flp.sharePointApplicationId%' THEN 'OK' ELSE 'NOT DEPLOYED' END;
...
CREATE OR ALTER PROCEDURE dbo.sel_flpConfigurationList    @processTypeId INT,
...
BEGIN
    ...
    BEGIN TRY
        SELECT
            ...
            sc.scheduleEndDate,
            sc.scheduleEndTime,
            -- SharePoint-only: side table fallback (see header comment)
            COALESCE(flp.sharePointApplicationId, sp_src.sharePointApplicationId) AS sharePointApplicationId,
            COALESCE(flp.sharePointApplicationSiteId, sp_src.sharePointApplicationSiteId) AS sharePointApplicationSiteId,
            COALESCE(flp.sharePointLibraryName, sp_src.sharePointLibraryName) AS sharePointLibraryName,
            COALESCE(flp.sharePointFolderPath, sp_src.sharePointFolderPath, flp.sourcePath) AS sharePointFolderPath
        ...
        -- SharePoint-only: side table (1:1 on flpConfigurationId — does not multiply rows)
        LEFT JOIN dbo.di_flpSharePointSource sp_src
