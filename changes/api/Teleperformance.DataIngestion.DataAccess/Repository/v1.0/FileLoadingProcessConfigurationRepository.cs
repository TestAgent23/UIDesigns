...
namespace Teleperformance.DataIngestion.DataAccess.Repository.v1._0
{
    public class FileLoadingProcessConfigurationRepository : IFileLoadingProcessConfigurationRepository
    {
        ...
        public async Task<DatabaseResponse> checkSharePointLandingFileStatus(string flpConfigurationId)
        {
            try
            {
                var dynamicParameters = new DynamicParameters();
                dynamicParameters.Add("@flpConfigurationId", flpConfigurationId);
                return await _dapperService.GetSingleRowAsync<DatabaseResponse>("[sel_checkSharePointLandingLayerUploadedFileStatus]", dynamicParameters, commandType: CommandType.StoredProcedure);
            }
            catch (Exception ex)
            {
                _logger.LogError(ex, ex.Message);
