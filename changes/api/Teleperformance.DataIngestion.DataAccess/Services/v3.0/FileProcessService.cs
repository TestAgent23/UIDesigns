            ...
            if (configurationResult.ResponseCode == 200)
            {
                ...
                else if (flpConfigurationRequestDto.LocationTypeId == (int)SourceLocationTypeEnum.SharePoint)
                {
                    if (!await FlpConfigurationHelper.TryPrepareSharePointFileAsync(_fileLoadingProcessConfiguration, flpRequestDto, flpConfigurationRequestDto, ".xlsx", ".xls", ".xlsb"))
                    {
                        ...
                        return new APIResponse<FlpProcessResponseDto> { ResultStatus = APIResultStatus.InvalidParameters, ResponseMessage = new List<string> { Teleperformance.DataIngestion.Sharepoint.Constants.Constants.InvalidOrMissingSharePointFile }, Result = null };
                    }
                }
                #endregion
