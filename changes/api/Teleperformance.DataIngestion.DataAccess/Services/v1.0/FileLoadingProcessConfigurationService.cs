...
namespace Teleperformance.DataIngestion.DataAccess.Services.v1._0
{
    public class FileLoadingProcessConfigurationService : IFileLoadingProcessConfiguration
    {
            ...
            foreach (var flp in flpConfigurations)
            {
                ...
                switch (flp.locationTypeId)
                {
                    ...
                    case (int)SourceLocationTypeEnum.SharePoint:
                        (dto.FileExistInSharePointLocation, dto.LandingLayerUploadedFileId) = await FileExistInSharePointLocation(
                            flp.flpConfigurationId, flp.sharePointApplicationId, flp.sharePointApplicationSiteId,
        ...
        var flpConfigurationList = new FlpConfigurationResponseDto
            ...
            #region Workspace Connect - SharePoint
            if (!flpConfigurationList.SharePointApplicationId.HasValue || flpConfigurationList.SharePointApplicationId.Value == Guid.Empty)
            {
                ...
                {
                    flpConfigurationList.SharePointApplicationId = sharePointSource.sharePointApplicationId;
                    flpConfigurationList.SharePointApplicationSiteId = sharePointSource.sharePointApplicationSiteId;
                    if (string.IsNullOrWhiteSpace(flpConfigurationList.SharePointLibraryName))
                        flpConfigurationList.SharePointLibraryName = sharePointSource.sharePointLibraryName;
                    if (string.IsNullOrWhiteSpace(flpConfigurationList.SharePointFolderPath))
                        flpConfigurationList.SharePointFolderPath = sharePointSource.sharePointFolderPath;
                }
            }
            #endregion
        ...
        #region Workspace Connect - SharePoint
        private async Task<bool> checkSharePointLandingLayerFileUploadedStatus(string flpConfigurationId)
        {
            var dbResult = await _fileLoadingProcessConfigurationRepository.checkSharePointLandingFileStatus(flpConfigurationId);
            if (string.Compare(dbResult.Result, "Success", true) == 0)
            {
                return true;
            }
            _logger.LogError($"SharePoint landing layer file status check failed for {flpConfigurationId}.");
            return false;
        }

        private async Task<List<SharePointFileLocation>> GetSharePointFileLocationList(string flpConfigurationId, Guid? sharePointApplicationId, Guid? sharePointApplicationSiteId, string? sharePointLibraryName, string? sharePointFolderPath, string searchStringInFileName, int processTypeId)
        {
            ...
            try
            {
                ...
                var credentials = CreateSharePointCredentials(sharePointApplicationId.Value, sharePointApplicationSiteId, sharePointLibraryName);
                var browsePath = string.IsNullOrWhiteSpace(sharePointFolderPath) ? "/" : sharePointFolderPath.Trim();
                var items = await _sharePointPluginService.BrowseFolderAsync(credentials, browsePath);

                foreach (var item in items)
                {
                    if (item.IsFolder || !await IsValidExtention(flpConfigurationId, item.Name)) continue;
                    if (!string.IsNullOrWhiteSpace(searchStringInFileName) && !item.Name.Contains(searchStringInFileName, StringComparison.OrdinalIgnoreCase)) continue;

                    (bool success, string uploadedId) = await UploadedFileDetails(flpConfigurationId, item.Name, processTypeId);
                    if (!success)
                    {
                        _logger.LogWarning("SharePoint file {FileName} was found but could not be registered for {FlpConfigurationId}", item.Name, flpConfigurationId);
                        continue;
                    }

                    sharePointFiles.Add(new SharePointFileLocation { UploadedId = uploadedId, FileUrl = BuildSharePointFilePath(item.Path, item.WebUrl, item.Name), FileName = item.Name, FileSize = item.Size });
                }
            }
            catch (Exception ex)
            {
                _logger.LogError(ex, "Error finding SharePoint files for {FlpConfigurationId}", flpConfigurationId);
            }

            return sharePointFiles;
        }

        private async Task<(bool fileExists, string uploadedFileId)> FileExistInSharePointLocation(string flpConfigurationId, Guid? sharePointApplicationId, Guid? sharePointApplicationSiteId, string? sharePointLibraryName, string? sharePointFolderPath, string searchStringInFileName, int processTypeId)
        {
            try
            {
                if (!sharePointApplicationId.HasValue || sharePointApplicationId.Value == Guid.Empty || !await checkSharePointLandingLayerFileUploadedStatus(flpConfigurationId))
                {
                ...
                {
                    if (item.IsFolder
                        || (!string.IsNullOrWhiteSpace(searchStringInFileName)
                            && !item.Name.Contains(searchStringInFileName, StringComparison.OrdinalIgnoreCase)))
                    {
                        continue;
                    }

                    if (await IsValidExtention(flpConfigurationId, item.Name))
                    {
                ...
                if (!matchingFileFound) return (false, string.Empty);

                (bool success, string uploadedId) = await UploadedFileDetails(flpConfigurationId, "Landing Layer File", processTypeId);
                return success ? (true, uploadedId) : (false, string.Empty);
            }
            catch (Exception ex)
            {
                _logger.LogError(ex, "Error checking SharePoint landing-layer files for {FlpConfigurationId}", flpConfigurationId);
                return (false, string.Empty);
            }
        }
        ...
        {
            var fileName = name?.Trim() ?? string.Empty;
            var normalizedPath = path?.Trim().Trim('/') ?? string.Empty;
            if (!string.IsNullOrEmpty(normalizedPath))
            {
                return normalizedPath.EndsWith(fileName, StringComparison.OrdinalIgnoreCase) ? normalizedPath : $"{normalizedPath}/{fileName}";
            }

            return !string.IsNullOrWhiteSpace(webUrl) ? webUrl.Trim() : fileName;
        }

        private static WorkspaceCredentials CreateSharePointCredentials(Guid applicationId, Guid? applicationSiteId, string? libraryName) => new() { ApplicationId = applicationId, ApplicationSiteId = applicationSiteId, LibraryName = string.IsNullOrWhiteSpace(libraryName) ? null : libraryName.Trim() };
        #endregion
    }
}
