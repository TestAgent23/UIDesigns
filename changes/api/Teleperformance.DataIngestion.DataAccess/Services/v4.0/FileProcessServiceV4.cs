    ...
    public class FileProcessServiceV4: IFileProcessServiceV4
    {
        ...
        public async Task<APIResponse<FlpDatabricksProcessResponseDto>> ProcessExcelFile(FlpRequestDto flpRequestDto)
        {
            ...
            if (configurationResult.ResponseCode == 200)
            {
                ...
                {
                    if (!await FlpConfigurationHelper.TryPrepareSharePointFileAsync(_fileLoadingProcessConfiguration, flpRequestDto, flpConfigurationRequestDto, ".xlsx", ".xls", ".xlsb"))
                    {
                        await _fileLoadingProcessConfiguration.UpdateFlpProcessStatus(flpConfigurationRequestDto.UploadedFileId ?? flpRequestDto.SharePointFileLocation?.UploadedId, APIResultStatus.Error);
                        return new APIResponse<FlpDatabricksProcessResponseDto> { ResultStatus = APIResultStatus.InvalidParameters, ResponseMessage = new List<string> { Teleperformance.DataIngestion.Sharepoint.Constants.Constants.InvalidOrMissingSharePointFile }, Result = null };
                    }
                }
                #endregion
        ...
        public async Task<APIResponse<FlpDatabricksProcessResponseDto>> ProcessCsvFile(FlpRequestDto flpRequestDto)
        {
            ...
            if (configurationResult.ResponseCode == 200)
            {
                ...
                {
                    if (!await FlpConfigurationHelper.TryPrepareSharePointFileAsync(_fileLoadingProcessConfiguration, flpRequestDto, flpConfigurationRequestDto, ".csv"))
                    {
                        await _fileLoadingProcessConfiguration.UpdateFlpProcessStatus(flpConfigurationRequestDto.UploadedFileId ?? flpRequestDto.SharePointFileLocation?.UploadedId, APIResultStatus.Error);
                        return new APIResponse<FlpDatabricksProcessResponseDto> { ResultStatus = APIResultStatus.InvalidParameters, ResponseMessage = new List<string> { Teleperformance.DataIngestion.Sharepoint.Constants.Constants.InvalidOrMissingSharePointFile }, Result = null };
                    }
        ...
        public async Task<APIResponse<FlpDatabricksProcessResponseDto>> ProcessTxtFile(FlpRequestDto flpRequestDto)
        {
            ...
            if (configurationResult.ResponseCode == 200)
            {
                ...
                {
                    if (!await FlpConfigurationHelper.TryPrepareSharePointFileAsync(_fileLoadingProcessConfiguration, flpRequestDto, flpConfigurationRequestDto, ".txt"))
                    {
                        await _fileLoadingProcessConfiguration.UpdateFlpProcessStatus(flpConfigurationRequestDto.UploadedFileId ?? flpRequestDto.SharePointFileLocation?.UploadedId, APIResultStatus.Error);
                        return new APIResponse<FlpDatabricksProcessResponseDto> { ResultStatus = APIResultStatus.InvalidParameters, ResponseMessage = new List<string> { Teleperformance.DataIngestion.Sharepoint.Constants.Constants.InvalidOrMissingSharePointFile }, Result = null };
                    }
        ...
        private async Task UpdateProcessStatus(FlpConfigurationResponseDto flpConfigurationRequestDto, APIResponse<FlpDatabricksProcessResponseDto> apiResponse)
        {
            ...
            if (apiResponse.ResultStatus.Code == APIResultStatus.Completed.Code)
            {
                ...
                else
                {
                    ...
                    await _databricksAPIDbREpository.UpdateLogHistoryStatus(processLogHistoryStatus);
                    #region Workspace Connect - SharePoint
                    if (flpConfigurationRequestDto.LocationTypeId == (int)SourceLocationTypeEnum.SharePoint
                        && !string.IsNullOrWhiteSpace(flpConfigurationRequestDto.UploadedFileId))
                    {
